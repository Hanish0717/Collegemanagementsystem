...
import { Settings, Bell, Brain, BarChart3, FileText, Save, RefreshCw, Shield, User } from "lucide-react";
import { Badge, Card, PageHeader } from "@/components/dashboard/ui";

export const Route = createFileRoute("/dashboard/ai/settings")({ component: AISettings });
    ...
    <div className="space-y-6">
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <Card>
          ...
          <div className="space-y-4">
            {[
              { label: "Enable AI Predictions", desc: "Allow AI to generate performance and attendance predictions", enabled: true },
              { label: "Risk Analysis", desc: "Enable automatic risk assessment for students", enabled: true },
              { label: "Smart Insights", desc: "Generate automated insights and recommendations", enabled: true },
              { label: "Chatbot Assistant", desc: "Enable AI-powered chatbot for queries", enabled: true },
            ].map((setting, index) => (
              <div key={index} className="flex items-center justify-between p-4 rounded-xl border">
                ...
                <div className={`w-12 h-6 rounded-full ${setting.enabled ? "bg-primary" : "bg-muted"} relative cursor-pointer`}>
                  <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${setting.enabled ? "left-7" : "left-1"}`} />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-4 p-4 border rounded-xl bg-gradient-soft">
            <div>
              ...
              <select className="w-full rounded-lg border bg-background px-3 py-2 text-sm">
                {["Daily", "Weekly", "Monthly", "On Demand"].map(freq => <option key={freq}>{freq}</option>)}
              </select>
            </div>
            <div>
              <label className="text-sm font-medium mb-2 block">Prediction Accuracy Threshold</label>
              <select className="w-full rounded-lg border bg-background px-3 py-2 text-sm">
                {["85%", "90%", "95%", "98%"].map(threshold => <option key={threshold}>{threshold}</option>)}
              </select>
            </div>
            <div>
              <label className="text-sm font-medium mb-2 block">Data Sources</label>
              <select className="w-full rounded-lg border bg-background px-3 py-2 text-sm">
                {["All Available Data", "Attendance Only", "Performance Only", "Custom Selection"].map(source => <option key={source}>{source}</option>)}
              </select>
            </div>
          </div>
        </Card>
      ...
      <Card>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            { label: "Risk Alerts", desc: "Receive notifications for high-risk students", enabled: true },
            { label: "Prediction Updates", desc: "Get notified when new predictions are generated", enabled: true },
            { label: "Attendance Warnings", desc: "Alerts for students below attendance threshold", enabled: true },
            ...
            <div key={index} className="p-4 rounded-xl border hover:bg-accent/50 transition">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className={`w-10 h-6 rounded-full ${setting.enabled ? "bg-primary" : "bg-muted"} relative cursor-pointer`}>
                  <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${setting.enabled ? "left-5" : "left-1"}`} />
                </div>
              </div>
              <div className="text-xs text-muted-foreground">{setting.desc}</div>
            </div>
      ...
      <Card>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            { label: "Auto-generate Reports", desc: "Automatically create reports on schedule", enabled: true },
            { label: "Weekly Summary", desc: "Generate weekly performance summaries", enabled: true },
            { label: "Monthly Analytics", desc: "Create comprehensive monthly reports", enabled: true },
            { label: "Risk Reports", desc: "Automated risk assessment reports", enabled: true },
            { label: "Attendance Reports", desc: "Regular attendance analysis reports", enabled: true },
      ...
      <Card>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            { label: "Dashboard View", desc: "Default dashboard layout preference", value: "Standard" },
            ...
            <div key={index} className="p-4 rounded-xl border hover:bg-accent/50 transition">
              ...
              <select className="w-full rounded-lg border bg-background px-3 py-1.5 text-xs">
                {[pref.value, "Option 2", "Option 3"].map(opt => <option key={opt}>{opt}</option>)}
              </select>
            </div>
          ))}
        </div>
      ...
      <Card>
        ...
        <div className="space-y-4">
          {[
            ...
            { label: "Training Data Last Updated", value: "May 20, 2026", desc: "Last date when training data was refreshed" },
            { label: "API Status", value: "Connected", desc: "Connection status to AI services" },
            { label: "Processing Queue", value: "0 pending", desc: "Number of pending AI processing tasks" },
          ].map((config, index) => (
            <div key={index} className="flex items-center justify-between p-4 rounded-xl border">
              <div>
      ...
      <div className="flex items-center justify-between p-4 rounded-xl border bg-gradient-soft">
        <div>
          ...
          <div className="text-xs text-muted-foreground">Restore all AI settings to default values</div>
        </div>
