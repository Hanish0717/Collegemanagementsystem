...
import { coursesManaged, departmentsManaged } from "@/lib/super-admin-data";
...
function CourseManagement() {
  ...
  const filtered = useMemo(() => coursesManaged.filter(course =>
    (department === "All" || course.department === department) &&
    [course.code, course.name, course.department].some(value => value.toLowerCase().includes(search.toLowerCase()))
  ), [department, search]);
      ...
      <Card>
        <div className="flex flex-col lg:flex-row gap-3">
          <div className="flex-1 relative">
            ...
            <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search courses..." className="w-full rounded-xl border bg-background/60 pl-10 pr-4 py-2.5 text-sm" />
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="border-b">
              <tr>
                {["Course Code", "Course Name", "Department", "Semester", "Credits", "Status", "Actions"].map(column => (
            ...
            <tbody className="divide-y">
              {filtered.map(course => (
                <tr key={course.code} className="hover:bg-accent/50 transition">
                  ...
                  <td className="py-3 px-4"><Badge tone="info">{course.department}</Badge></td>
                  <td className="py-3 px-4 text-muted-foreground">{course.semester}</td>
                  <td className="py-3 px-4 font-medium">{course.credits}</td>
                  <td className="py-3 px-4">
                    <Badge tone={course.status === "Active" ? "success" : course.status === "Review" ? "warn" : "danger"}>{course.status}</Badge>
                  </td>
                  <td className="py-3 px-4">
      ...
      <Card>
        ...
        <div className="space-y-4 p-4 border rounded-xl bg-gradient-soft">
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <input placeholder="Course code" className="rounded-lg border bg-background px-3 py-2 text-sm" />
            <input placeholder="Course name" className="rounded-lg border bg-background px-3 py-2 text-sm" />
            <select className="rounded-lg border bg-background px-3 py-2 text-sm">
              {departmentsManaged.map(dept => <option key={dept.id}>{dept.name}</option>)}
            </select>
            <select className="rounded-lg border bg-background px-3 py-2 text-sm">
              {["Semester 1", "Semester 2", "Semester 3", "Semester 4", "Semester 5", "Semester 6"].map(item => <option key={item}>{item}</option>)}
            </select>
            <input placeholder="Credits" type="number" className="rounded-lg border bg-background px-3 py-2 text-sm" />
            <select className="rounded-lg border bg-background px-3 py-2 text-sm">
              {["Active", "Review", "Inactive"].map(item => <option key={item}>{item}</option>)}
            </select>
          </div>
