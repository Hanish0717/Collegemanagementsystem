...
import { events } from "@/lib/student-data";
    ...
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-4 gap-4">
        {[
          ...
          { label: "Registered", value: events.filter(e => e.status === "Registered").length.toString(), tone: "success" as const },
          ...
          {events.map(event => (
            <Card key={event.id} className="hover:-translate-y-1 transition">
              <div className="flex items-start justify-between mb-4">
                ...
                <Badge tone={event.status === "Registered" ? "success" : "warn"}>{event.status}</Badge>
              </div>
      ...
      <Card>
        ...
        <div className="space-y-2">
          {events.filter(e => e.status === "Registered").map(event => (
            <div key={event.id} className="flex items-center gap-3 p-3 rounded-xl border hover:bg-accent/50 transition">
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-muted-foreground">{event.type} • {event.date}</div>
              </div>
