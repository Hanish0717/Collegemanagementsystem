...
import { Download, FileText, BarChart3, TrendingUp, Calendar, Clock, CheckCircle } from "lucide-react";
import { Badge, Card, PageHeader } from "@/components/dashboard/ui";

export const Route = createFileRoute("/dashboard/ai/reports")({ component: SmartReports });
...
function SmartReports() {
  ...
  const recentReports = [
    { name: "Monthly Attendance Report - May 2026", type: "Attendance", date: "May 25, 2026", size: "2.4 MB", status: "Ready" },
    { name: "Academic Performance - Semester 5", type: "Academic", date: "May 24, 2026", size: "3.1 MB", status: "Ready" },
    { name: "Department Analysis - Computer Science", type: "Department", date: "May 23, 2026", size: "1.8 MB", status: "Ready" },
    { name: "Placement Statistics - 2026", type: "Placement", date: "May 22, 2026", size: "4.2 MB", status: "Processing" },
    { name: "Student Risk Summary - May", type: "Risk", date: "May 21, 2026", size: "2.9 MB", status: "Ready" },
  ];
  ...
  const insights = [
    { title: "Attendance Improvement", desc: "Overall college attendance improved by 3.2% this month", icon: TrendingUp },
    { title: "Academic Excellence", desc: "Computer Science department shows highest GPA improvement", icon: CheckCircle },
    { title: "Risk Reduction", desc: "High-risk student count decreased by 8% after interventions", icon: TrendingUp },
  ];
  ...
  return (
    <div className="space-y-6">
        ...
        <Card>
          ...
          <div className="grid grid-cols-2 gap-4">
            {reportTypes.map((report, index) => (
              <div key={index} className="p-4 rounded-xl border hover:bg-accent/50 transition cursor-pointer">
                <div className={`size-12 rounded-lg ${report.color} text-white grid place-items-center mb-3`}>
              ...
              {recentReports.map((report, index) => (
                <tr key={index} className="hover:bg-accent/50 transition">
                  ...
                  <td className="py-3 px-4">
                    <Badge tone={report.status === "Ready" ? "success" : "warn"}>{report.status}</Badge>
                  </td>
                  <td className="py-3 px-4">
      ...
      <Card>
        ...
        <div className="space-y-3">
          {[
            { date: "May 25, 2026", time: "10:30 AM", report: "Monthly Attendance Report", status: "Completed" },
            { date: "May 24, 2026", time: "03:45 PM", report: "Academic Performance Report", status: "Completed" },
            { date: "May 23, 2026", time: "11:20 AM", report: "Department Analysis Report", status: "Completed" },
            { date: "May 22, 2026", time: "02:15 PM", report: "Placement Statistics Report", status: "Processing" },
            { date: "May 21, 2026", time: "09:00 AM", report: "Student Risk Summary", status: "Completed" },
          ].map((history, index) => (
            <div key={index} className="flex items-center gap-3 p-3 rounded-xl border hover:bg-accent/50 transition">
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-muted-foreground">{history.date} • {history.time}</div>
              </div>
              <Badge tone={history.status === "Completed" ? "success" : "warn"}>{history.status}</Badge>
            </div>
          ))}
        </div>
      </Card>
      ...
      <Card>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 p-4 border rounded-xl bg-gradient-soft">
          <select className="rounded-lg border bg-background px-3 py-2 text-sm">
            {["Select Report Type", "Attendance", "Academic", "Placement", "Department", "Risk Analysis"].map(type => <option key={type}>{type}</option>)}
          </select>
          <select className="rounded-lg border bg-background px-3 py-2 text-sm">
            {["Select Time Period", "This Month", "Last Month", "This Semester", "This Year", "Custom Range"].map(period => <option key={period}>{period}</option>)}
          </select>
          <select className="rounded-lg border bg-background px-3 py-2 text-sm">
            {["Select Department", "All Departments", "Computer Science", "Electrical Engineering", "Mechanical Engineering", "Business Administration"].map(dept => <option key={dept}>{dept}</option>)}
          </select>
      ...
      <Card>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: "Total Downloads", value: "1,245", icon: Download, color: "bg-gradient-primary" },
            { label: "This Week", value: "89", icon: Download, color: "bg-gradient-violet" },
            { label: "Most Downloaded", value: "Attendance", icon: FileText, color: "bg-gradient-cyan" },
            { label: "Avg Download Time", value: "2.3s", icon: Clock, color: "bg-gradient-primary" },
          ].map((stat, index) => (
            <div key={index} className="p-4 rounded-xl border hover:bg-accent/50 transition">
              <div className="flex items-center gap-2 mb-2">
                <div className={`size-8 rounded-lg ${stat.color} text-white grid place-items-center`}>
