...
export function AiInsights() {
  const insights = [
    {
      ...
      desc: 'Overall college attendance improved by 3.2% this month after implementing new monitoring system',
      type: 'Positive',
      impact: 'High',
    },
    {
      title: 'Academic Excellence',
      desc: 'Computer Science department shows highest GPA improvement of 0.4 points',
      type: 'Positive',
      impact: 'High',
    },
    {
      title: 'Risk Reduction',
      desc: 'High-risk student count decreased by 8% after targeted interventions',
      type: 'Positive',
      impact: 'Medium',
    },
    {
      title: 'Engagement Alert',
      desc: 'Student participation in extracurricular activities dropped by 5%',
      type: 'Negative',
      impact: 'Medium',
    },
    {
      title: 'Performance Trend',
      desc: 'Mathematics department showing consistent improvement across all semesters',
      type: 'Positive',
      impact: 'High',
    },
    {
      title: 'Resource Optimization',
      desc: 'Library utilization increased by 15% suggesting better resource management',
      type: 'Positive',
      impact: 'Low',
    },
  ];
  ...
  const recommendations = [
    {
      title: 'Increase Tutoring Support',
      desc: 'Assign additional tutors for Mathematics and Physics departments',
      priority: 'High',
    },
    {
      title: 'Enhance Engagement Programs',
      desc: 'Launch new student engagement initiatives to boost participation',
      priority: 'Medium',
    },
    {
      title: 'Monitor Attendance Patterns',
      desc: 'Implement real-time attendance alerts for at-risk students',
      priority: 'High',
    },
    {
      title: 'Optimize Class Schedules',
      desc: 'Adjust timetable based on student performance patterns',
      priority: 'Medium',
    },
    {
      title: 'Strengthen Parent Communication',
      desc: 'Increase frequency of parent-teacher meetings',
      priority: 'Low',
    },
    {
      title: 'Expand Mentorship Program',
      desc: 'Pair high-performing students with those needing support',
      priority: 'High',
    },
  ];
        ...
        <Card>
          ...
          <div className="space-y-3">
            {insights.map((insight, index) => (
              <div
                key={index}
                className={`p-4 rounded-xl border hover:bg-accent/50 transition ${insight.type === 'Negative' ? 'bg-red-50 border-red-200' : ''}`}
              >
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium">{insight.title}</span>
                  <div className="flex items-center gap-2">
                    <Badge tone={insight.type === 'Positive' ? 'success' : 'warn'}>
                      {insight.type}
                    </Badge>
                    <Badge tone={insight.impact === 'High' ? 'warn' : 'info'}>
          ...
          {highlights.map((highlight, index) => (
            <div key={index} className="p-4 rounded-xl border hover:bg-accent/50 transition">
              <div className="flex items-center gap-2 mb-2">
                <div
                  className={`size-8 rounded-lg ${highlight.color} text-white grid place-items-center`}
                >
      ...
      <Card>
        ...
        <div className="space-y-3">
          {[
            {
              time: '10:30 AM',
              insight: 'Attendance improvement detected in Computer Science department',
              type: 'Positive',
            },
            {
              time: '09:45 AM',
              insight: 'Risk assessment completed for 245 students',
              type: 'Analysis',
            },
            {
              time: '09:15 AM',
              insight: 'Performance trend analysis shows upward trajectory',
              type: 'Positive',
            },
            {
              time: '08:30 AM',
              insight: 'New recommendation generated for Mathematics department',
              type: 'Recommendation',
            },
            {
              time: '08:00 AM',
              insight: 'Daily insight generation completed successfully',
              type: 'System',
            },
          ].map((item, index) => (
            <div
          ...
          {[
            {
              ...
              desc: 'Participation in activities dropped by 5%',
              priority: 'Medium',
            },
            {
              title: 'Mechanical Engineering',
              desc: 'Performance score decreased by 3 points',
              priority: 'High',
            },
            {
              title: 'Library Resources',
              desc: 'Book return rate below expected levels',
              priority: 'Low',
            },
          ].map((area, index) => (
            <div
              key={index}
              className={`p-4 rounded-xl border hover:bg-accent/50 transition ${area.priority === 'High' ? 'bg-amber-50 border-amber-200' : ''}`}
            >
              <div className="flex items-center justify-between mb-2">
