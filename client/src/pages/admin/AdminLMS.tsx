...
export function AdminLMS() {
  ...
  const [resources, setResources] = useState<ResourceItem[]>([
    {
      id: 'RES-101',
      title: 'Syllabus Plan & Course Outcomes',
      type: 'PDF Document',
      subject: 'Machine Learning Foundations',
      size: '1.2 MB',
    },
    {
      id: 'RES-102',
      title: 'Neural Networks Backpropagation Guide',
      type: 'PDF Document',
      subject: 'Machine Learning Foundations',
      size: '3.4 MB',
    },
    {
      id: 'RES-103',
      title: 'Lexical Analyzer Code Templates',
      type: 'PDF Document',
  ...
  const [videos, setVideos] = useState<ResourceItem[]>([
    ...
    {
      id: 'VID-02',
      title: 'LL(1) Parser Table Construction Tutorial',
      type: 'Video Lecture',
      subject: 'Compiler Design',
      size: '32 Mins',
      url: 'https://www.youtube.com/embed/smnL5M-nNlo',
    },
  ]);
  ...
  const [assignments, setAssignments] = useState<AssignmentItem[]>([
    {
      ...
      status: 'Pending',
      description:
        'Implement batch, stochastic, and mini-batch gradient descent algorithms from scratch and compare their convergence curves on the housing price dataset.',
    },
    {
      id: 'ASG-02',
      ...
      status: 'Submitted',
      description:
        'Build a parser in C++/Java that parses custom grammar for mathematical operators accounting for parenthesis, operator priority, and associativity.',
      submittedFile: 'https://example.com/recursive-descent-parser.pdf',
    },
    {
      id: 'ASG-03',
      ...
      status: 'Graded',
      description:
        'Write comparative notes on matrix chain multiplication and find the optimal parenthesis order using both bottom-up and top-down DP paradigms.',
      grade: 'A+',
      feedback: 'Excellent documentation and trace tables!',
      submittedFile: 'https://example.com/dp-assignment.pdf',
    },
  ]);
  ...
  const [quizzes, setQuizzes] = useState<QuizItem[]>([
    {
      ...
      questions: [
        {
          question:
            'Which activation function is widely used to prevent vanishing gradient in deep nets?',
          options: ['Sigmoid', 'Tanh', 'ReLU', 'Linear'],
          correct: 2,
        },
        {
          question: 'What is the primary function of learning rate in gradient descent?',
          options: [
            'Determines the batch sizing',
            'Controls the step size in update iterations',
            'Configures layer nodes',
    ...
    },
    {
      ...
      questions: [
        {
          question: 'An LR parser stands for:',
          options: [
            'Left-to-right, Rightmost derivation in reverse',
            'Leftmost derivation, Recursive descent',
            'Lexical parsing, Regular parsing',
            'Linear resolution, Right parsing',
          ],
          correct: 0,
        },
      ],
  ...
  const [forumPosts, setForumPosts] = useState<ForumPost[]>([
    {
      ...
      date: '1 day ago',
      content:
        "Hi all, I am building a predictor model but keep overfitting. Does max_depth or min_samples_split help more with regularization in sklearn's RandomForestClassifier?",
      comments: [
        {
          author: 'Dr. Srinivas Rao',
          role: 'Faculty',
          content:
            'Reducing max_depth and increasing min_samples_leaf will both regularize effectively by limiting split depth.',
          date: '12 hours ago',
        },
      ],
    },
    {
      ...
      date: '2 days ago',
      content:
        'The syllabus scope for Compiler Design midterms next week will cover lexical analysis and LL(1)/LR(0) parsing. Please review resources RES-103.',
      comments: [],
    },
  ]);
  const [activePost, setActivePost] = useState<ForumPost | null>(null);
  const [newPostTitle, setNewPostTitle] = useState('');
  const [newPostCategory, setNewPostCategory] = useState<'General' | 'Doubts' | 'Announcements'>(
    'Doubts',
  );
  ...
  const [classesList, setClassesList] = useState<ClassItem[]>([
    {
      id: 'CLS-01',
      subject: 'Machine Learning Foundations',
      topic: 'Regularization Techniques & L1/L2 Norms',
    ...
    },
    {
      id: 'CLS-02',
      subject: 'Compiler Design',
      topic: 'Syntax-Directed Translation Schemes',
    ...
    },
    {
      id: 'CLS-03',
      subject: 'Design & Analysis of Algorithms',
      topic: 'Network Flow & Ford-Fulkerson Algorithm',
      instructor: 'Dr. Srinivas Rao',
      date: 'July 20',
      time: '01:00 PM - 02:00 PM',
      link: 'https://meet.google.com/mnp-qrst-uvw',
      status: 'Upcoming',
    },
  ]);
      ...
      prev.map((a) =>
        a.id === submittingAssignmentId
          ? {
              ...a,
              status: 'Submitted',
              submittedFile: submitFileLink || 'https://example.com/submitted-notes.pdf',
            }
          : a,
      ),
    );
      ...
      prev.map((a) =>
        a.id === gradingSubId
          ? {
              ...
              status: 'Graded',
              grade: gradeInput || 'A',
              feedback: feedbackInput || 'Good effort!',
            }
          : a,
      ),
    );
  ...
  const handleQuizSubmit = () => {
    ...
    setQuizzes((prev) =>
      prev.map((q) =>
        q.id === activeQuiz.id ? { ...q, completed: true, score: finalPercentage } : q,
      ),
    );
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <Card>
            ...
            <ul className="text-xs text-slate-600 space-y-2 mt-3 list-disc pl-4 leading-relaxed">
              <li>
                <strong>Study Notes</strong> tab contains official PDFs uploaded by HODs and
                faculty.
              </li>
              <li>
                <strong>Video Lectures</strong> features interactive classroom lecture records.
              </li>
              <li>
                Always check the <strong>Assignments Hub</strong> to submit outstanding worksheets
                before due dates.
              </li>
              <li>
                Post your queries on the <strong>Discussion Forum</strong> to get peer answers.
              </li>
            </ul>
          </Card>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <Card className="lg:col-span-2">
            ...
            <div className="space-y-3">
              {resources
                .filter((r) => r.type === 'PDF Document')
                .map((row) => (
                  <div
                    key={row.id}
                    className="p-3 border rounded-xl flex items-center justify-between text-xs bg-slate-50/50 hover:bg-slate-50 transition"
                  >
                    <div>
                      <div className="font-bold text-slate-800">{row.title}</div>
                      <div className="text-[10px] text-slate-400 mt-0.5">
                        Course: {row.subject} • PDF Document
                      </div>
                    </div>
            ...
            <Card>
              ...
              <form onSubmit={handlePublishNote} className="space-y-3">
                ...
                <div>
                  ...
                  >
                    {syllabusList.map((s) => (
                      <option key={s.code} value={s.subject}>
                        {s.subject}
                      </option>
                    ))}
                  </select>
            ...
            {activeVideo ? (
              <div className="space-y-3">
                ...
                <p className="text-xs text-muted-foreground">
                  Subject: {activeVideo.subject} • Duration: {activeVideo.size}
                </p>
              </div>
            ) : (
              <div className="grid sm:grid-cols-2 gap-4">
                {videos.map((vid) => (
                  <div
                    key={vid.id}
                    className="border rounded-2xl overflow-hidden hover:shadow-md transition bg-white flex flex-col justify-between"
                  >
                    <div
                      className="aspect-video w-full bg-slate-950 flex items-center justify-center relative cursor-pointer group"
            ...
            {viewingSubmissionsAsgId ? (
              <div className="space-y-4">
                ...
                <h3 className="font-bold text-slate-800 text-sm">
                  Submissions for:{' '}
                  {assignments.find((a) => a.id === viewingSubmissionsAsgId)?.title}
                </h3>
                ...
                <div className="space-y-3 mt-3">
                  {[
                    {
                      student: 'Hanish Kumar',
                      file: 'https://example.com/hanish-ml.pdf',
                      submittedAt: 'July 15, 2026',
                      status:
                        assignments.find((a) => a.id === viewingSubmissionsAsgId)?.status ===
                        'Graded'
                          ? 'Graded'
                          : 'Submitted',
                      grade: assignments.find((a) => a.id === viewingSubmissionsAsgId)?.grade,
                      feedback: assignments.find((a) => a.id === viewingSubmissionsAsgId)?.feedback,
                    },
                    {
                      student: 'Aarav Sharma',
                      file: 'https://example.com/aarav-ml.pdf',
                      submittedAt: 'July 14, 2026',
                      status: 'Submitted',
                    },
                  ].map((sub, idx) => (
                    <div
                      key={idx}
                      className="p-4 border rounded-2xl text-xs space-y-2 bg-slate-50/50"
                    >
                      <div className="flex justify-between items-center font-bold">
                        <span className="text-slate-800">{sub.student}</span>
                        <Badge tone={sub.status === 'Graded' ? 'success' : 'info'}>
                          {sub.status}
                        </Badge>
                      </div>
                      ...
                      {sub.status === 'Graded' ? (
                        <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3 mt-2 text-[11px] text-slate-800 space-y-1">
                          <div>
                            <strong>Grade:</strong>{' '}
                            <span className="font-bold text-emerald-700">{sub.grade}</span>
                          </div>
                          <div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {assignments.map((asg) => (
                    <div
                      key={asg.id}
                      className="p-4 border rounded-2xl text-xs space-y-3 hover:shadow-xs transition bg-white"
                    >
                      ...
                      {asg.status === 'Graded' && (
                        <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3 text-[11px] text-slate-800 space-y-1">
                          ...
                          <div>
                            Grade Score: <span className="font-bold">{asg.grade}</span>
                          </div>
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 border rounded-2xl bg-slate-50/50">
                  ...
                  <p className="text-xs text-slate-400">
                    Subject: {activeQuiz.subject} • {activeQuiz.questions.length} questions
                  </p>
                </div>
                ...
                {quizScore !== null ? (
                  <div className="bg-indigo-50 border border-indigo-100 rounded-3xl p-6 text-center space-y-3">
                    ...
                    <p className="text-xs text-slate-500">
                      Your score represents correctness of all choices selected.
                    </p>
              ...
              <div className="space-y-4">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {quizzes.map((qz) => (
                    <div
                      key={qz.id}
                      className="p-4 border rounded-2xl hover:shadow-xs transition bg-white flex flex-col justify-between gap-4"
                    >
                      <div>
            ...
            {activePost ? (
              <div className="space-y-4">
                ...
                <div className="p-4 border rounded-2xl bg-slate-50/50 space-y-2">
                  ...
                  <p className="text-xs text-slate-600 pt-2 border-t leading-relaxed whitespace-pre-line">
                    {activePost.content}
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  {forumPosts.map((p) => (
                    ...
                    >
                      ...
                      <h4 className="font-bold text-xs text-slate-800 hover:text-indigo-600 transition">
                        {p.title}
                      </h4>
          ...
          <Card className="lg:col-span-2">
            ...
            <div className="space-y-4">
              {classesList.map((cls) => (
                <div
                  key={cls.id}
                  className="p-4 border rounded-2xl flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white hover:shadow-xs transition"
                >
        ...
        {gradingSubId && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center z-50 p-4">
            ...
            >
              ...
              <p className="text-xs text-slate-400 mb-4">
                Provide a grade score and feedback remarks for the student's submission.
              </p>
