...
export function AdminInventory() {
  ...
  const [gatePasses, setGatePasses] = useState([
    {
      id: 'GP-001',
      item: 'Dell OptiPlex 7090 Desktop',
      qty: 2,
      sentTo: 'Dell Service Center, Hyd',
      status: 'Outbound',
      date: '2026-07-16',
    },
    {
      ...
      qty: 1,
      sentTo: 'Keysight Lab Instruments',
      status: 'Returned',
      date: '2026-07-10',
    },
  ]);
  const [assets, setAssets] = useState([
    ...
    {
      id: 'AST-103',
      name: 'Keysight DSO Oscilloscope',
      category: 'Lab Equipment',
      lab: 'ECE Circuits Lab',
      qty: 12,
      status: 'Healthy',
    },
    {
      id: 'AST-104',
      name: 'Network Cisco Switches 24P',
      category: 'Networking',
      lab: 'Server Room',
      qty: 8,
      status: 'Healthy',
  ...
  const [purchaseOrders, setPurchaseOrders] = useState([
    {
      id: 'PO-901',
      vendor: 'Prime Computers Inc',
      items: '15x Core i7 Desktops',
      cost: '₹7.5L',
      status: 'Pending approval',
    },
    {
      id: 'PO-902',
      vendor: 'Electronic Emporium',
      items: '20x VLSI Breadboards',
      cost: '₹45,000',
      status: 'Pending approval',
    },
  ]);
              ...
              <div className="space-y-3">
                {purchaseOrders.length === 0 ? (
                  <div className="text-xs text-muted-foreground text-center py-6">
                    All pending orders dispatched!
                  </div>
                ) : (
                  purchaseOrders.map((row) => (
                    <div
                      key={row.id}
                      className="p-3 border rounded-xl space-y-2 text-xs hover:bg-slate-50 transition"
                    >
        ...
        <Card>
          <div className="flex justify-between items-center mb-3">
            <h3 className="font-semibold text-slate-800 text-sm">
              Asset Depreciation Ledger &amp; WDV Valuation
            </h3>
                ...
                {depreciationLedger.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="py-3 text-center">
                      <Badge tone="warn">{item.rate}</Badge>
                    </td>
      ...
      {activeTab === 'gatepasses' && (
        <div className="grid lg:grid-cols-3 gap-6">
          <Card className="lg:col-span-2">
            <div className="flex justify-between items-center mb-3">
              <h3 className="font-semibold text-slate-800 text-sm">
                Outward Gate Passes (Hardware Repair/Calibration)
              </h3>
                ...
                >
                  <div className="space-y-1">
                    ...
                    <div className="text-slate-500 font-semibold">
                      Sent To: {pass.sentTo} | Date: {pass.date}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge tone={pass.status === 'Returned' ? 'success' : 'warn'}>
                      {pass.status}
                    </Badge>
                    {pass.status !== 'Returned' && (
                      <button
                        onClick={() => {
                          setGatePasses((prev) =>
                            prev.map((p) => (p.id === pass.id ? { ...p, status: 'Returned' } : p)),
                          );
