...
export function AdminCommunication() {
  ...
  const [polls, setPolls] = useState([
    {
      id: 'POLL-01',
      question: 'Rate the quality of online study materials',
      responses: 482,
      status: 'Active',
      deadline: '2026-07-25',
    },
    {
      id: 'POLL-02',
      question: 'Preferences for the winter vacation schedule',
      responses: 914,
      status: 'Closed',
      deadline: '2026-07-10',
    },
  ...
  const [broadcastLogs, setBroadcastLogs] = useState([
    {
      id: 'MSG-101',
      subject: 'Odd Term Examinations Fee Deadline',
      target: 'All Students',
      channel: 'Portal Circular',
      sentCount: 5240,
      status: 'Delivered',
      time: '2 hours ago',
    },
    {
      id: 'MSG-102',
      subject: 'Syllabus Audit Meeting - Dean Office',
      target: 'All Faculty',
      channel: 'Email & Push',
      sentCount: 342,
      status: 'Delivered',
    ...
    {
      id: 'MSG-103',
      subject: 'Attendance Defaulter Warning Notice',
      target: 'CSE Parents',
  ...
  const handleSendBroadcast = (e: React.FormEvent) => {
    ...
    toast.success(
      `Broadcast successfully queued! Dispatching via ${comChannel} to ${count.toLocaleString()} recipients.`,
    );
    setMsgSubject('');
    setMsgBody('');
  };
    ...
    <div className="space-y-6">
          ...
          <div className="grid lg:grid-cols-3 gap-4">
            ...
            <Card className="flex flex-col justify-between">
              ...
              <div className="p-3 bg-indigo-50 border border-indigo-100 rounded-xl text-[10px] text-indigo-700 leading-normal mt-4">
                <strong>System Sync:</strong> All circular postings automatically inject directly
                into the Student, Faculty, and Parent dashboard timeline widgets.
              </div>
            </Card>
          </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {broadcastLogs.map((row) => (
                    <tr key={row.id}>
                      ...
                      <td className="py-2.5 text-center font-mono font-bold text-indigo-600">
      ...
      {activeTab === 'polls' && (
        <div className="grid lg:grid-cols-3 gap-6">
          <Card className="lg:col-span-2">
            <h3 className="font-semibold text-slate-800 text-sm mb-3">
              Institutional Feedback Surveys &amp; Student Polls
            </h3>
            <div className="space-y-3.5">
              {polls.map((poll) => (
                <div
                ...
                >
                  <div className="space-y-1">
                    ...
                    <div className="text-slate-500 font-semibold">
                      Total Submissions: {poll.responses} responses
                    </div>
                    <div className="text-[10px] text-slate-400 font-bold">
                      Closing Deadline: {poll.deadline}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge tone={poll.status === 'Active' ? 'success' : 'default'}>
                      {poll.status}
                    </Badge>
                    {poll.status === 'Active' && (
                      <button
