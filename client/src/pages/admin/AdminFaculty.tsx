    ...
    () =>
      faculty.filter(
        (fac) =>
          ...
          [fac.id, fac.name, fac.department, fac.subject].some((value) =>
    ...
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-4 gap-4">
        {[
          ...
          {
            label: "On Leave",
            value: faculty.filter((f) => f.status === "On Leave").length.toString(),
            tone: "warn" as const,
          },
          {
            label: "Departments",
            value: departmentDistributionAdmin.length.toString(),
            tone: "info" as const,
          },
        ].map((stat) => (
          <Card key={stat.label}>
              ...
              {filtered.map((fac) => (
                <tr key={fac.id} className="hover:bg-accent/50 transition">
                  ...
                  <td className="py-3 px-4">
                    <Badge tone="info">{fac.department}</Badge>
                  </td>
