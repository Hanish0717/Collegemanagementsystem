...
export function AdminAccreditation() {
  ...
  const [aqarReports, setAqarReports] = useState([
    ...
    },
    {
      ...
      compiledBy: 'Mrs. Ananya Sen',
      status: 'Drafting',
      submissionDate: 'Pending (Due Dec 2026)',
    },
  ]);
  ...
  const [ssrDocs, setSsrDocs] = useState([
    {
      id: 'SSR-DOC-001',
      criteria: 'Criteria 1.1.3',
      title: 'Syllabus revision documents & board minutes',
      file: 'BoardMinutes_2025.pdf',
      status: 'Approved',
    },
    {
      id: 'SSR-DOC-002',
      criteria: 'Criteria 2.2.1',
      title: 'Student-computer ratio verification statement',
      file: 'ComputerInventory_2026.pdf',
  ...
  const [nbaStatus, setNbaStatus] = useState([
    ...
    {
      course: 'B.Tech Electrical & Electronics Eng',
      status: 'Re-evaluation Pending',
      validity: 'Expired Nov 2025',
    },
  ]);

  const handleGenerateSSR = () => {
    toast.loading('Compiling self-study report indices and verification documents...', {
      duration: 1500,
    });
    setTimeout(() => {
      toast.success(
  ...
  const handleOBESync = () => {
    toast.success(
      'Outcome-Based Education (OBE) Course Outcomes (CO) mapping verified against Program Outcomes (PO) successfully!',
    );
  };
    ...
    <div className="space-y-6">
            ...
            <Card className="lg:col-span-2">
              <div className="flex justify-between items-center mb-4">
                <div>
                  <h3 className="font-semibold text-slate-800 text-sm">
                    NAAC Criteria Progress Checklist
                  </h3>
                  <p className="text-[10px] text-slate-500">
                    Quality score compliance breakdown across 6 Criteria modules.
                  </p>
                </div>
                <button
              ...
              <div className="space-y-3.5">
                {naacCriteria.map((row) => (
                  <div
                    key={row.id}
                    className="text-xs space-y-1.5 p-3 border rounded-xl bg-slate-50/50"
                  >
                    <div className="flex justify-between font-semibold text-slate-800">
                      ...
                      <span className="font-mono text-indigo-700 font-bold">
                        {row.score} (Complied: {row.progress}%)
                      </span>
                    </div>
            ...
            <Card className="flex flex-col justify-between">
              <div>
                <h3 className="font-semibold text-slate-800 text-sm mb-3">
                  NBA OBE Department Status
                </h3>
                <div className="space-y-3">
                  {nbaStatus.map((row) => (
                    <div
                      key={row.course}
                      className="p-3 border rounded-xl space-y-1 text-xs hover:bg-slate-50 transition"
                    >
              ...
              <tbody className="divide-y divide-slate-100">
                {aqarReports.map((report) => (
                  <tr key={report.id}>
                    ...
                    <td className="py-3 text-right font-mono text-slate-500 font-semibold">
      ...
      {activeTab === 'ssr' && (
        <Card>
          <div className="flex justify-between items-center mb-3">
            <h3 className="font-semibold text-slate-800 text-sm">
              Self-Study Report (SSR) Supporting Data Vault
            </h3>
