...
export function AdminFacultyAttendance() {
  ...
  const handleStatusChange = (id: string, newStatus: string) => {
    setFacultyList(prev => prev.map(f => f.faculty.id === id ? { ...f, status: newStatus } : f));
  };

  const handleRemarksChange = (id: string, newRemarks: string) => {
    setFacultyList(prev => prev.map(f => f.faculty.id === id ? { ...f, remarks: newRemarks } : f));
  };
  ...
  const handleUpdateRecordStatus = async (recordId: string, newStatus: string, remarks: string, facultyIdForReload?: string) => {
    try {
          ...
          <Card>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                >
                  {["CSE", "AIML", "AIDS", "ECE", "EEE", "MECH", "CIVIL", "IT"].map(d => <option key={d} value={d}>{d}</option>)}
                </select>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="border-b">
                    <tr>
                      {["Employee ID", "Faculty Name", "Designation", "Overall %", "Attendance Status", "Remarks"].map(
                        (column) => (
                          <th
                            key={column}
                  ...
                  <tbody className="divide-y">
                    {filteredFaculty.length > 0 ? (
                      filteredFaculty.map(f => {
                        ...
                        return (
                          <tr key={f.faculty.id} className={`hover:bg-accent/50 transition ${shortage ? "bg-red-50/30" : ""}`}>
                            ...
                            <td className="py-3 px-4 text-xs text-muted-foreground">{f.faculty.designation}</td>
          ...
          <Card>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Search a faculty member by name or employee ID to retrieve their complete calendar log.</p>
              </div>
            </div>
          ...
          {selectedFacultyForHistory ? (
            ...
            <Card>
              <div className="flex items-center justify-between border-b pb-4 mb-4">
                <div>
                  <h4 className="font-bold text-lg text-foreground">{selectedFacultyForHistory.full_name || selectedFacultyForHistory.fullName}</h4>
                  <p className="text-xs text-muted-foreground">{selectedFacultyForHistory.employee_id || selectedFacultyForHistory.employeeId} | {selectedFacultyForHistory.designation} | Department of {selectedFacultyForHistory.department}</p>
                </div>
                <div className="text-right">
                  <Badge tone={(selectedFacultyForHistory.attendance_percentage || 100) >= 80 ? "success" : "danger"} className="text-sm px-3 py-1 font-bold">
                    {selectedFacultyForHistory.attendance_percentage || selectedFacultyForHistory.attendancePercentage || 100}% Overall
                  </Badge>
                  <div className="text-[10px] text-muted-foreground mt-1">
                    {(selectedFacultyForHistory.attendance_percentage || 100) >= 80 ? "Regular Attendance" : "Attendance Shortage"}
                  </div>
                </div>
              </div>
                        ...
                        <tr key={rec._id || rec.id} className="hover:bg-accent/50 transition">
                          ...
                          <td className="py-3 px-4">
                            <select
                              ...
                              onChange={(e) => handleUpdateRecordStatus(rec._id || rec.id, e.target.value, rec.remarks || "", selectedFacultyForHistory.employee_id || selectedFacultyForHistory.employeeId)}
                            ...
                            <input
                              ...
                              onBlur={(e) => {
                                if (e.target.value !== (rec.remarks || "")) {
                                  handleUpdateRecordStatus(rec._id || rec.id, rec.statusDisplay, e.target.value, selectedFacultyForHistory.employee_id || selectedFacultyForHistory.employeeId);
                                }
                              }}
                          ...
                          <td className="py-3 px-4">
                            <button
                              onClick={() => handleDeleteRecord(rec._id || rec.id, selectedFacultyForHistory.employee_id || selectedFacultyForHistory.employeeId)}
            ...
            <Card>
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b pb-4 mb-4">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Verify and modify saved logs for all department staff on {selectedDate}.</p>
                </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="border-b">
                    <tr>
                      {["Employee ID", "Faculty Name", "Overall %", "Current Logged Status", "Remarks", "Actions"].map((col) => (
                        <th key={col} className="text-left py-3 px-4 font-semibold text-muted-foreground">{col}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y">
                    {facultyList.length > 0 ? (
                      facultyList.map((f) => {
                        ...
                        return (
                          <tr key={f.faculty.id} className={`hover:bg-accent/50 transition ${shortage ? "bg-red-50/30" : ""}`}>
                            <td className="py-3 px-4 font-medium text-xs">{f.faculty.employeeId}</td>
                            <td className="py-3 px-4 font-medium">{f.faculty.fullName}</td>
                            <td className="py-3 px-4">
                              <span className={`font-semibold text-xs px-2 py-0.5 rounded-full ${shortage ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700"}`}>
                                {f.faculty.attendancePercentage}%
                            ...
                            <td className="py-3 px-4 text-xs text-muted-foreground">{f.remarks || "-"}</td>
                            <td className="py-3 px-4 flex items-center gap-2">
                              {hasRecord ? (
                                <>
                                  <button
                                    onClick={() => {
                                      const promptStatus = window.prompt("Enter new status (Present, Absent, Late, Excused):", f.status);
                                      if (promptStatus) {
                                        const clean = promptStatus.trim();
                                        if (["Present", "Absent", "Late", "Excused"].includes(clean)) {
                                          handleUpdateRecordStatus(f.id || f._id, clean, f.remarks || "");
                                        } else {
                                ...
                                <span className="text-xs text-muted-foreground italic">Use taking tab</span>
                              )}
                            </td>
                          </tr>
                        );
