  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-4 gap-4">
        {[
          {
            label: 'Total Students Audited',
            value: String(students.length),
            tone: 'info' as const,
          },
          {
            label: 'Eligible Automatically',
            value: String(
              students.filter((s) => s.attendance >= 75 && s.feeStatus === 'No Due').length,
            ),
            tone: 'success' as const,
          },
          {
            label: 'Blocked / Ineligible',
            value: String(students.filter((s) => !s.eligible).length),
            tone: 'danger' as const,
          },
          {
            label: 'Manual Overrides Active',
            value: String(
              students.filter((s) => s.eligible && (s.attendance < 75 || s.feeStatus !== 'No Due'))
                .length,
            ),
            tone: 'warn' as const,
          },
                ...
                return (
                  <tr key={s.id} className="hover:bg-accent/40 transition">
                    ...
                    <td className="py-4 px-4">
                      <span
                        className={`text-xs font-semibold px-2 py-0.5 rounded-full ${s.attendance < 75 ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'}`}
                      >
                        {s.attendance}%
                      </span>
                    </td>
                    <td className="py-4 px-4 text-xs font-semibold">
                      <span
                        className={`${s.feeStatus === 'Due Pending' ? 'text-red-600' : 'text-emerald-600'}`}
                      >
                        {s.feeStatus}
                      </span>
                    </td>
