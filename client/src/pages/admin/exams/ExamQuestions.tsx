          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="border-b">
                <tr>
                  {['Subject & Code', 'Uploader / Faculty', 'Diff Level', 'Status', 'Actions'].map(
                    (col) => (
                      <th
                        key={col}
              ...
              <tbody className="divide-y">
                {papers.map((p) => (
                  <tr key={p.id} className="hover:bg-accent/40 transition">
                    ...
                    <td className="py-4 px-4">
                      <span
                        className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${p.difficulty === 'Hard' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'}`}
                      >
                        {p.difficulty}
                      </span>
                    </td>
                    <td className="py-4 px-4">
                      <Badge tone={p.status === 'Approved' ? 'success' : 'warn'}>{p.status}</Badge>
                    </td>
