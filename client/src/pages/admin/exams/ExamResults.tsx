...
export function ExamResults() {
  const [results, setResults] = useState([
    ...
    {
      id: 'R03',
      name: 'Semester 1 Practical Results',
      department: 'MECH',
      batch: '2025-2029',
      studentsCount: 40,
                    ...
                    {r.status === 'Awaiting Moderation' && (
                      <>
                        <button
                          ...
                          className="px-2 py-1 bg-amber-50 border rounded text-amber-600 text-xs font-bold transition hover:bg-amber-100 flex items-center gap-1"
                        >
                          <RefreshCw className="size-3" /> Moderate
                        </button>
                        <button
                          onClick={() => handlePublish(r.id)}
                          className="px-2 py-1 bg-emerald-600 border rounded text-white text-xs font-bold transition hover:bg-emerald-700"
                        >
                          Publish
                        </button>
                      </>
