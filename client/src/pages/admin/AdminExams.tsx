...
export function AdminExams() {
  ...
  const { data: activeTimetable = [], isLoading: isTimetableLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await api.get<{ success: boolean; data: Schedule[] }>(
  ...
  const {
    ...
    isLoading: isEligLoading,
    refetch: refetchEligibility,
  } = useQuery({
    queryKey: ['exams', selectedExamId, 'hall-tickets'],
    queryFn: async () => {
      if (!selectedExamId) return [];
      const { data } = await api.get<{ success: boolean; data: StudentEligibility[] }>(
  ...
  const { data: examAnalytics, isLoading: isAnalyticsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await api.get<{ success: boolean; data: any }>(
        `/api/exams/${selectedExamId}/analytics`,
      );
      return data.data;
    },
  ...
  const approveHallTicketMutation = useMutation({
    mutationFn: async ({
      ...
      await api.post(`/api/exams/${id}/hall-tickets/approve`, {
  ...
  const downloadHallTicketPDF = (student: StudentEligibility) => {
    ...
    doc.setFontSize(8.5);
    doc.text(
      '1. Candidates must carry this Hall Ticket and College ID Card to the exam hall.',
      20,
      instrY + 6,
    );
    doc.text(
      '2. Please report to the designated hall 15 minutes before the start time.',
      20,
      instrY + 11,
    );
    doc.text(
      '3. Electronic gadgets, smartwatches, and mobile phones are strictly prohibited.',
      20,
      instrY + 16,
    );
    ...
    <div className="space-y-6 pb-12">
                        ...
                        <td className="px-4 py-3 text-center flex items-center justify-center gap-1.5">
                          ...
                          <button
                            onClick={() => {
                              if (
                                confirm(
                                  'Are you sure you want to delete this scheduled exam? All associated timetable records will be permanently removed.',
                                )
                              ) {
        ...
        <div className="space-y-6">
          ...
          <Card>
            <label className="text-xs font-semibold text-muted-foreground block mb-2">
              Select Examination to Build Timetable
            </label>
            <select
                  ...
                  <div className="space-y-4 p-4 border rounded-2xl bg-gradient-soft">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setTimetableForm({ ...timetableForm, date: e.target.value })
                        }
                        className="w-full mt-1.5 rounded-lg border bg-background px-3 py-2 text-xs outline-none focus:border-primary"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setTimetableForm({ ...timetableForm, hall: e.target.value })
                        }
                        className="w-full mt-1.5 rounded-lg border bg-background px-3 py-2 text-xs outline-none focus:border-primary"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setTimetableForm({ ...timetableForm, duration: e.target.value })
                        }
                        className="w-full mt-1.5 rounded-lg border bg-background px-3 py-2 text-xs outline-none focus:border-primary"
                      />
                    </div>
                ...
                <Card className="h-full flex flex-col">
                  ...
                  {isTimetableLoading ? (
                    ...
                    <div className="flex-1 flex items-center justify-center text-xs text-muted-foreground border-2 border-dashed border-muted rounded-2xl py-12">
                      No subject schedules added to this exam timetable yet. Use the left panel to
                      add slots.
                    </div>
                  ) : (
        ...
        <Card>
          <div className="flex justify-between items-center mb-3">
            <h3 className="font-semibold text-slate-800 text-sm">
              Question Bank Vault &amp; Peer Auditing
            </h3>
                        ...
                        <>
                          <button
                            onClick={() => {
                              setQbank((prev) =>
                                prev.map((q) =>
                                  q.id === row.id ? { ...q, status: 'Approved' } : q,
                                ),
                              );
                              toast.success(
                                `Question Bank for ${row.subject} approved successfully!`,
                              );
                            }}
                          ...
                          <button
                            onClick={() => {
                              setQbank((prev) =>
                                prev.map((q) =>
                                  q.id === row.id ? { ...q, status: 'Rejected' } : q,
                                ),
                              );
                              toast.error(
                                `Question Bank for ${row.subject} rejected/sent back for revision!`,
                              );
                            }}
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <Card className="lg:col-span-2">
            <h3 className="font-semibold text-slate-800 text-sm mb-3">
              Invigilator Allocation &amp; Exam Hall Duties
            </h3>
                ...
                >
                  <div className="space-y-1">
                    ...
                    <div className="text-slate-500 font-semibold">
                      Date: {duty.date} | Session: {duty.session}
                    </div>
                    <div className="text-[10px] text-slate-400 font-bold">
                      Assigned Location: {duty.hall}
                    </div>
                  </div>
                  <button
                    onClick={() => {
      ...
      {activeTab === 'halltickets' && (
        <div className="space-y-6">
          ...
          <Card>
            <label className="text-xs font-semibold text-muted-foreground block mb-2">
              Select Examination for Hall Tickets Audit
            </label>
            <select
              value={selectedExamId}
              onChange={(e) => setSelectedExamId(e.target.value)}
          ...
          {selectedExamId && (
            <Card>
              <h3 className="font-semibold mb-4 text-gradient text-xs">
                Student Hall Tickets Eligibility
              </h3>
              {isEligLoading ? (
                <div className="flex flex-col items-center justify-center py-12 gap-2">
                  <Loader2 className="size-8 text-primary animate-spin" />
                ...
                <div className="text-center py-12 text-xs text-muted-foreground">
                  No students found enrolled in the exam's cohort ({selectedExam?.department} - Year{' '}
                  {selectedExam?.year} / Sem {selectedExam?.semester}).
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                      ...
                      {eligibilityList.map((s) => (
                        <tr key={s.id} className="hover:bg-muted/30">
                          ...
                          <td className="px-4 py-3">
                            ...
                            <Badge
                              tone={s.attendanceEligible ? 'success' : 'danger'}
                          ...
                          <td className="px-4 py-3">
                            ...
                            <Badge
                              tone={s.feeEligible ? 'success' : 'danger'}
                              className="scale-90 origin-left mt-1"
                            >
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1">
                              ...
                              <span
                                className={`font-semibold ${s.eligible ? 'text-emerald-600' : 'text-rose-600'}`}
                              >
                          ...
                          <td className="px-4 py-3 text-center">
                            {s.status === 'Approved' ? (
                              ...
                              <button
                                onClick={() =>
                                  approveHallTicketMutation.mutate({
                                    id: selectedExamId,
                                    studentId: s.id,
                                  })
                                }
      ...
      {activeTab === 'results' && (
        <div className="space-y-6">
                    ...
                    <button
                      onClick={() =>
                        updateExamMutation.mutate({
                          id: selectedExamId,
                          updates: { status: 'Results Published' },
                        })
                      }
          ...
          {selectedExamId && selectedSubject && (
            <Card>
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-semibold text-gradient text-xs">
                  Enter Student Marks ({selectedSubject})
                </h3>
                <button
                  onClick={() =>
                  ...
                  disabled={
                    saveResultsMutation.isPending || selectedExam?.status === 'Results Published'
                  }
      ...
      {activeTab === 'analytics' && (
        <div className="space-y-6">
          ...
          <Card>
            <label className="text-xs font-semibold text-muted-foreground block mb-2">
              Select Examination for Analytics
            </label>
            <select
              value={selectedExamId}
              onChange={(e) => setSelectedExamId(e.target.value)}
            ...
            <div className="grid lg:grid-cols-3 gap-6">
              {isAnalyticsLoading ? (
                ...
                <div className="col-span-3 text-center py-12 text-xs text-muted-foreground border-2 border-dashed rounded-2xl">
                  No graded marks submissions found for this exam. Please upload marks in the
                  "Results Publisher" tab.
                </div>
              ) : (
                <>
                  {/* Left stats columns */}
                  <div className="lg:col-span-1 space-y-4">
                    <Card>
                      <h3 className="font-semibold mb-4 text-xs text-muted-foreground uppercase">
                        ...
                        <div className="p-4 rounded-xl bg-gradient-soft border flex justify-between items-center">
                          <div>
                            ...
                            <div className="text-2xl font-bold text-indigo-600 mt-1">
                              {examAnalytics.averageMarks} / 100
                            </div>
                          </div>
                        ...
                        <div className="p-4 rounded-xl bg-gradient-soft border flex justify-between items-center">
                          <div>
                            ...
                            <div className="text-2xl font-bold text-foreground mt-1">
                              {examAnalytics.totalSubmissions}
                            </div>
                          </div>
                    ...
                    <Card>
                      ...
                      <div className="space-y-3">
                        {examAnalytics.topStudents.map((stud: any, idx: number) => (
                          <div
                            key={idx}
                            className="flex justify-between items-center p-3 rounded-xl border bg-gradient-soft"
                          >
                            <div>
                  ...
                  <div className="lg:col-span-2">
                    <Card className="h-full flex flex-col justify-between">
                          ...
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart
                              data={[
                                {
                                  name: 'Pass Rate',
                                  value: examAnalytics.passPercentage,
                                  fill: '#10B981',
                                },
                                {
                                  name: 'Fail Rate',
                                  value: examAnalytics.failPercentage,
                                  fill: '#EF4444',
                                },
                              ]}
                            >
                      ...
                      <div className="p-3 border rounded-xl bg-indigo-50 border-indigo-100 text-indigo-700 text-xs mt-4">
                        💡 **Note:** Pass eligibility requires student subject score $\ge 40$. Stats
                        auto-refresh upon publication of grades.
                      </div>
                    </Card>
                  </div>
                </>
