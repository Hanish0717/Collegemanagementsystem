...
export function EventGalleryTab({ eventList }: any) {
  const placeholders = [
    'Grand Alumni Reunion 2024',
    'Silver Jubilee Dinner',
    'Annual Tech Talk',
    'Batch 2010 Get-Together',
    'Homecoming Weekend',
    'Industry Networking Night',
    'Campus Walk 25th Anniversary',
    'Cultural Evening 2023',
  ];
  const colors = [
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Visual memories from alumni events and reunions
          </p>
        </div>
...
export function InternshipsTab({ alumniList }: any) {
  ...
  const internships = [
    ...
    },
    {
      ...
      skills: ['Product', 'Analytics', 'Agile'],
      batch: 2020,
      deadline: '2024-10-01',
      type: 'Management',
    },
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Internships shared by alumni from top companies
          </p>
        </div>
        ...
        {filtered.map((item) => (
          <Card key={item.id} className="flex flex-col gap-3">
            <div className="flex items-start justify-between">
              ...
              <span
                className={`text-[10px] font-bold px-2 py-1 rounded-full ${typeColors[item.type] ?? 'bg-gray-100 text-gray-700'}`}
              >
                {item.type}
              </span>
            </div>
            <div className="flex flex-wrap gap-1">
              {item.skills.map((s) => (
                <span
                  key={s}
                  className="text-[10px] px-2 py-0.5 bg-muted rounded-full text-muted-foreground font-medium"
                >
                  {s}
                </span>
              ))}
            ...
            <div className="pt-2 border-t flex items-center justify-between">
              ...
              <button className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-[11px] font-bold rounded-lg cursor-pointer transition">
                Apply Now
              </button>
            </div>
          </Card>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Batch-wise placement statistics and top recruiters
        </p>
      </div>
      ...
      <Card>
        ...
        <div className="flex flex-wrap gap-3">
          {recruiters.map((r, i) => (
            <div
              key={i}
              className="flex items-center gap-2 px-4 py-2 border rounded-xl bg-muted/30 hover:bg-muted/60 transition cursor-pointer"
            >
              <div className="size-6 rounded-full bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-white text-[10px] font-bold">
                {r[0]}
              </div>
  ...
  const faqs = [
    {
      ...
      a: 'Go to Alumni Management → Alumni Registration and fill in your graduation details. Your profile will be reviewed and approved by the coordinator.',
    },
    {
      q: 'How can I post a job for current students?',
      a: "Navigate to Career → Job Portal and click 'Post New Job'. Fill in the job details and it will be visible to all registered students.",
    },
    {
      q: 'How do I donate to the alumni fund?',
      a: 'Visit Contributions → Donations, choose your preferred donation amount and payment method. All donations are tax-deductible under Section 80G.',
    },
    {
      q: 'How does the mentorship program work?',
      a: 'Register in Community → Mentorship as a mentor. Students and juniors can request sessions. You can approve or decline and schedule via the calendar.',
    },
    {
      q: 'How do I update my profile?',
      a: 'Go to Alumni Management → Alumni Profiles. Select your profile and click Edit to update your current job, location, skills, and social links.',
    },
    {
      q: 'Can I see who else is in my batch?',
      a: 'Yes! In Alumni Management → Alumni Directory, use the Graduation Year filter to see all members of your batch.',
    },
  ];
    ...
    <div className="space-y-6 max-w-3xl">
      <div className="rounded-2xl bg-gradient-to-r from-indigo-600 to-violet-600 p-6 text-white">
        ...
        <p className="text-white/80 text-sm">
          Find answers to common questions or contact the alumni team for assistance.
        </p>
      </div>
      <Card>
          ...
          {faqs.map((faq, i) => (
            <div key={i} className="border rounded-xl p-4 hover:bg-muted/30 transition">
              <div className="font-semibold text-sm mb-1.5 flex items-start gap-2">
                <span className="size-5 rounded-full bg-indigo-100 text-indigo-700 text-[10px] font-bold flex items-center justify-center shrink-0 mt-0.5">
                  {i + 1}
                </span>
                {faq.q}
              </div>
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          {[
            ...
            <div key={c.label} className="flex items-center gap-3 p-3 border rounded-xl">
              <div className={`p-2 rounded-lg ${c.cls}`}>
