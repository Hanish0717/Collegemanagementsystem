...
export function AdminStudents() {
  ...
  const [activeTab, setActiveTab] = useState<'roster' | 'discipline' | 'achievements' | 'vault'>(
    'roster',
  );
  ...
  const [disciplinaryLogs, setDisdisciplinaryLogs] = useState([
    {
      ...
      student: 'Amit Verma',
      roll: '21CS001',
      violation: 'Littering inside Lab',
    ...
    },
    {
      ...
      roll: '21CS002',
      violation: 'Attendance Shortage',
      action: 'Parent Memo Sent',
      date: '2026-07-10',
      severity: 'Medium',
    },
  ]);
  ...
  const [achievements, setAchievements] = useState([
    {
      ...
      roll: '21CS003',
      title: 'Smart India Hackathon First Prize',
      category: 'Technical',
      date: '2026-06-15',
    },
    {
      ...
      student: 'Aman Sharma',
      roll: '21CS004',
      title: 'State Inter-Collegiate Basketball Winners',
      category: 'Sports',
      date: '2026-06-28',
    },
  ]);
  ...
  const [sisTab, setSisTab] = useState<
    'profile' | 'history' | 'certificates' | 'parent' | 'idcard' | 'alumni'
  >('profile');
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid md:grid-cols-4 gap-4">
            {[
              ...
              },
              {
                ...
                value: warningStudentsCount.toString(),
                tone: 'warn' as const,
              },
                      ...
                      {filteredStudents.slice(0, 10).map((student) => {
                        const deptObj =
                          typeof student.department === 'object' && student.department
                            ...
                            : deptList.find(
                                (d) =>
                                  d.code === student.department || d._id === student.department,
                              );
                        const deptName = deptObj
                          ? deptObj.name
            ...
            <Card className="flex flex-col justify-center items-center text-center p-6 space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground max-w-sm mt-1">
                  Add new student registrations directly in MongoDB. Realtime links will associate
                  students to matching section/subject faculty advisors automatically.
                </p>
              </div>
              <button
                onClick={() => {
        ...
        <div className="grid lg:grid-cols-3 gap-4">
          <Card className="lg:col-span-2">
            <h3 className="font-semibold text-slate-800 text-sm mb-3">
              Conduct Log &amp; Disciplinary Warnings
            </h3>
                ...
                >
                  <div className="space-y-1">
                    ...
                    <div className="text-[10px] text-slate-400 font-bold">
                      Action Taken: {log.action} | Date: {log.date}
                    </div>
                  </div>
                </div>
              ))}
      ...
      {activeTab === 'achievements' && (
        <Card>
          <h3 className="font-semibold text-slate-800 text-sm mb-3">
            Student Extra-Curricular &amp; Academic Achievements
          </h3>
          <div className="overflow-x-auto">
                ...
                {achievements.map((ach) => (
                  <tr key={ach.id}>
                    ...
                    <td className="py-3 text-center">
                      <Badge tone="success">{ach.category}</Badge>
                    </td>
      ...
      {activeTab === 'vault' && (
        <Card>
          <h3 className="font-semibold text-slate-800 text-sm mb-3">
            Student Certificate Vault &amp; Documents Checklist
          </h3>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                    ...
                    <tr key={row.roll}>
                        ...
                        <input
                          ...
                          onChange={() => {
                            setVaultRecords((prev) =>
                              prev.map((r) =>
                                r.roll === row.roll ? { ...r, marksheets10: !r.marksheets10 } : r,
                              ),
                            );
                      ...
                      <td className="py-3 text-center">
                        <input
                          ...
                          onChange={() => {
                            setVaultRecords((prev) =>
                              prev.map((r) =>
                                r.roll === row.roll ? { ...r, marksheets12: !r.marksheets12 } : r,
                              ),
                            );
                            toast.success('Document vault status toggled!');
                          }}
                      ...
                      <td className="py-3 text-center">
                        <input
                          ...
                          onChange={() => {
                            setVaultRecords((prev) =>
                              prev.map((r) => (r.roll === row.roll ? { ...r, tc: !r.tc } : r)),
                            );
                            toast.success('Document vault status toggled!');
                          }}
                          className="rounded text-indigo-600 size-3 cursor-pointer"
                      ...
                      <td className="py-3 text-center">
                        <input
                          ...
                          onChange={() => {
                            setVaultRecords((prev) =>
                              prev.map((r) =>
                                r.roll === row.roll ? { ...r, incomeCert: !r.incomeCert } : r,
                              ),
                            );
                            toast.success('Document vault status toggled!');
                          }}
              ...
              <form onSubmit={handleOtpSubmit} className="space-y-4">
                <div className="text-center py-4">
                  <p className="text-sm text-muted-foreground mb-4">
                    ...
                    enter the 6-digit code to verify the account and complete registration.
                  </p>
                  {otpError && (
      ...
      {selectedSisStudent &&
        (() => {
          const deptObj =
            typeof selectedSisStudent.department === 'object' && selectedSisStudent.department
              ? selectedSisStudent.department
              : deptList.find(
                  (d: any) =>
                    d.code === selectedSisStudent.department ||
                    d._id === selectedSisStudent.department,
                );
          const sisDeptName = deptObj
            ? deptObj.name
            : typeof selectedSisStudent.department === 'string'
              ? selectedSisStudent.department
              : 'Other';
                ...
                <div className="flex items-center justify-between p-5 border-b bg-muted/20">
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground mt-0.5">
                      Detailed registry files for {selectedSisStudent.fullName} (
                      {selectedSisStudent.rollNumber})
                    </p>
                  </div>
                  <button
                    type="button"
                        ...
                        <div className="flex items-center gap-3 pb-3 border-b">
                          ...
                          <div>
                            <h4 className="font-bold text-sm text-slate-800">
                              {selectedSisStudent.fullName}
                            </h4>
                            <p className="text-[10px] text-muted-foreground">
                              Roll: {selectedSisStudent.rollNumber} | Adm:{' '}
                              {selectedSisStudent.admissionNumber || 'N/A'}
                            </p>
                          </div>
                          ...
                          <div className="bg-slate-50 p-3 rounded-xl border">
                            ...
                            <div className="mt-1.5 space-y-1.5">
                              <div className="flex justify-between">
                                ...
                                <span className="font-bold text-slate-800">{sisDeptName}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-muted-foreground">Academic Year:</span>{' '}
                                <span className="font-semibold">
                                  Year {selectedSisStudent.year}
                                </span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-muted-foreground">Semester:</span>{' '}
                                <span className="font-semibold">
                                  Semester {selectedSisStudent.semester}
                                </span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-muted-foreground">Section:</span>{' '}
                                <span className="font-semibold">
                                  Section {selectedSisStudent.section || 'A'}
                                </span>
                              </div>
                            </div>
                          </div>
                            ...
                            <div className="mt-1.5 space-y-1.5">
                              <div className="flex justify-between">
                                ...
                                <span className="font-semibold truncate max-w-[120px]">
                                  {selectedSisStudent.email}
                                </span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-muted-foreground">Phone:</span>{' '}
                                <span className="font-semibold">
                                  {selectedSisStudent.phoneNumber || '9876543210'}
                                </span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-muted-foreground">Gender:</span>{' '}
                                <span className="font-semibold">
                                  {selectedSisStudent.gender || 'Male'}
                                </span>
                              </div>
                              <div className="flex justify-between">
                    ...
                    {sisTab === 'history' && (
                      <div className="space-y-4">
                        ...
                        <div className="border rounded-2xl overflow-hidden bg-background">
                          <div className="px-3 py-2 bg-muted/20 border-b text-[10px] font-bold uppercase text-muted-foreground">
                            Semester Breakdown
                          </div>
                          <table className="w-full text-xs text-left">
                            ...
                            <tbody className="divide-y">
                              <tr>
                                <td className="p-2 font-semibold">Semester 1</td>
                                <td className="p-2 font-bold text-slate-700">8.45</td>
                                <td className="p-2">22 Credits</td>
                                <td className="p-2 text-emerald-600 font-bold">Passed</td>
                              </tr>
                              <tr>
                                <td className="p-2 font-semibold">Semester 2</td>
                                <td className="p-2 font-bold text-slate-700">8.70</td>
                                <td className="p-2">22 Credits</td>
                                <td className="p-2 text-emerald-600 font-bold">Passed</td>
                              </tr>
                              <tr>
                                <td className="p-2 font-semibold">Semester 3</td>
                                <td className="p-2 font-bold text-slate-700">8.32</td>
                                <td className="p-2">20 Credits</td>
                                <td className="p-2 text-emerald-600 font-bold">Passed</td>
                              </tr>
                              <tr>
                                <td className="p-2 font-semibold">Semester 4</td>
                                <td className="p-2 font-bold text-slate-700">8.85</td>
                                <td className="p-2">20 Credits</td>
                                <td className="p-2 text-emerald-600 font-bold">Passed</td>
                              </tr>
                            </tbody>
                    ...
                    {sisTab === 'certificates' && (
                      <div className="space-y-3">
                        <h4 className="font-bold text-xs text-slate-700 mb-2">
                          Student Certificate Verification &amp; Issuance
                        </h4>
                        {[
                          ...
                          {
                            name: 'Transcript of Marks (Consolidated)',
                            status: 'Issued',
                            type: 'system',
                          },
                        ].map((cert, idx) => (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-3">
                              <Badge tone={cert.status === 'Issued' ? 'success' : 'warn'}>
                                {cert.status}
                              </Badge>
                              <button
                                onClick={() =>
                                  toast.success(
                                    `Generated and downloaded ${cert.name} for ${selectedSisStudent.fullName}!`,
                                  )
                                }
                        ...
                        <div className="p-4 border rounded-2xl bg-indigo-50/20 space-y-3 text-xs">
                          <div>
                            ...
                            <div className="font-bold text-slate-800 mt-0.5 text-sm">
                              {selectedSisStudent.parentName || 'Ramesh Verma'}
                            </div>
                          </div>
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <div className="font-semibold text-slate-700 mt-0.5">
                                {selectedSisStudent.parentPhone || '9876543211'}
                              </div>
                            </div>
                            <div>
                              <span className="text-[10px] font-bold text-muted-foreground uppercase">
                              ...
                              <div className="font-semibold text-slate-700 mt-0.5">
                                {selectedSisStudent.parentEmail || 'parent@gmail.com'}
                              </div>
                            </div>
                          </div>
                          <div>
                            ...
                            <div className="font-semibold text-slate-700 mt-0.5">
                              Flat 402, Royal Residency, Vizianagaram, AP, 535002
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col items-center gap-4">
                        ...
                        <div className="w-[340px] h-[200px] rounded-2xl border bg-blue-900 text-white p-4 relative overflow-hidden shadow-lg select-none">
                          ...
                          <div className="flex items-center gap-1.5 border-b border-indigo-800 pb-2">
                            ...
                            <Badge className="ml-auto text-[8px] bg-cyan-500 hover:bg-cyan-500 text-white border-0 py-0 px-1">
                              STUDENT
                            </Badge>
                          </div>
                          ...
                          <div className="flex items-center gap-3.5 mt-3.5">
                            ...
                            <div className="space-y-0.5 text-left min-w-0">
                              <div className="font-extrabold text-sm truncate max-w-[180px]">
                                {selectedSisStudent.fullName}
                              </div>
                              <div className="text-[9px] text-indigo-300 font-mono">
                                Roll: {selectedSisStudent.rollNumber}
                              </div>
                              <div className="text-[9px] text-indigo-300 font-semibold">
                                {sisDeptName}
                              </div>
                              <div className="text-[8px] text-cyan-400 font-medium pt-1">
                                Valid: July 2024 - June 2028
                              </div>
                            </div>
                          </div>
                        ...
                        <button
                          onClick={() =>
                            toast.success(
                              `Student ID Card triggered for print! (Roll No: ${selectedSisStudent.rollNumber})`,
                            )
                          }
                      ...
                      <div className="space-y-4">
                        <div className="p-4 border rounded-2xl bg-slate-50 space-y-4 text-xs">
                          <div>
                            ...
                            <p className="text-[10px] text-muted-foreground mt-0.5">
                              Toggle student cohort categorization to mark graduation and place
                              placement tracks.
                            </p>
                          </div>
                          ...
                          <div className="flex items-center justify-between p-3 border bg-background rounded-xl">
                            <div className="space-y-0.5">
                              ...
                              <div className="text-[10px] text-muted-foreground">
                                Mark student as graduated alumnus.
                              </div>
                            </div>
                            <button
                              onClick={() => {
                                toast.success(`${selectedSisStudent.fullName} marked as Alumnus!`);
                                setSelectedSisStudent((prev: any) => ({
                                  ...prev,
                                  year: 4,
                                  semester: 8,
                                }));
