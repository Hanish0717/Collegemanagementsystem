...
export function AdminHRMS() {
  const [activeTab, setActiveTab] = useState<"records" | "recruitment" | "attendance" | "leave" | "payroll" | "performance">("records");

  // State: Employee Records
  const [employees, setEmployees] = useState([
    { id: "EMP-101", name: "Dr. Srinivas Rao", dept: "CSE", designation: "Professor", status: "Active", leaveBalance: 18, email: "srinivas.rao@college.edu", joined: "2018-06-15" },
    { id: "EMP-102", name: "Mrs. Ananya Sen", dept: "CSE", designation: "Assistant Professor", status: "Active", leaveBalance: 12, email: "ananya.sen@college.edu", joined: "2021-08-20" },
    { id: "EMP-103", name: "Dr. Aisha Khan", dept: "IT", designation: "HOD IT", status: "Active", leaveBalance: 15, email: "aisha.khan@college.edu", joined: "2017-04-10" },
    { id: "EMP-104", name: "Mr. Ramesh Yadav", dept: "CSE", designation: "Senior Lecturer", status: "Active", leaveBalance: 14, email: "ramesh.yadav@college.edu", joined: "2020-11-05" }
  ]);

  // State: Recruitment
  const [recruitmentList, setRecruitmentList] = useState([
    { id: "REC-01", title: "Assistant Professor (AIML)", depts: "AIML", applicants: 24, status: "Interviewing", type: "Full-Time" },
    { id: "REC-02", title: "System Administrator", depts: "IT", applicants: 15, status: "Open", type: "Full-Time" },
    { id: "REC-03", title: "Lab Assistant (ECE)", depts: "ECE", applicants: 8, status: "Open", type: "Contract" }
  ]);

  // State: Attendance
  const [biometricLogs, setBiometricLogs] = useState([
    { id: "BIO-1001", employee: "Dr. Srinivas Rao", time: "2026-07-16 08:58 AM", punchType: "Check-In", device: "Main Block Gate 1", status: "Present" },
    { id: "BIO-1002", employee: "Mrs. Ananya Sen", time: "2026-07-16 09:02 AM", punchType: "Check-In", device: "CSE Dept Gate A", status: "Present" },
    { id: "BIO-1003", employee: "Mr. Ramesh Yadav", time: "2026-07-16 09:15 AM", punchType: "Check-In", device: "Main Block Gate 1", status: "Late" }
  ]);

  // State: Leave
  const [leaveRequests, setLeaveRequests] = useState([
    { id: "LR-501", employee: "Dr. Srinivas Rao", type: "Casual Leave", start: "2026-07-20", end: "2026-07-22", days: 3, status: "Pending" },
    { id: "LR-502", employee: "Mrs. Ananya Sen", type: "Sick Leave", start: "2026-07-18", end: "2026-07-19", days: 2, status: "Approved" },
    { id: "LR-503", employee: "Mr. Ramesh Yadav", type: "Earned Leave", start: "2026-07-25", end: "2026-07-30", days: 5, status: "Pending" }
  ]);

  // State: Payroll
  const [payrollLogs, setPayrollLogs] = useState([
    { id: "PAY-701", employee: "Dr. Srinivas Rao", basic: 95000, allowance: 12000, deductions: 5000, net: 102000, status: "Disbursed" },
    { id: "PAY-702", employee: "Mrs. Ananya Sen", basic: 65000, allowance: 8000, deductions: 3000, net: 70000, status: "Pending" },
    { id: "PAY-703", employee: "Dr. Aisha Khan", basic: 110000, allowance: 15000, deductions: 6000, net: 119000, status: "Disbursed" },
    { id: "PAY-704", employee: "Mr. Ramesh Yadav", basic: 55000, allowance: 6000, deductions: 2500, net: 58500, status: "Pending" }
  ]);

  // State: Performance
  const [performanceRecords, setPerformanceRecords] = useState([
    { id: "PRF-801", employee: "Dr. Srinivas Rao", rating: 4.8, publications: 14, feedbackScore: "Excellent", targetAchieved: "95%" },
    { id: "PRF-802", employee: "Mrs. Ananya Sen", rating: 4.2, publications: 3, feedbackScore: "Good", targetAchieved: "88%" },
    { id: "PRF-803", employee: "Dr. Aisha Khan", rating: 4.9, publications: 22, feedbackScore: "Outstanding", targetAchieved: "98%" },
    { id: "PRF-804", employee: "Mr. Ramesh Yadav", rating: 3.9, publications: 2, feedbackScore: "Satisfactory", targetAchieved: "82%" }
  ]);
  ...
  const handleAddEmployee = (e: React.FormEvent) => {
    ...
    setPayrollLogs(prev => [
      ...prev,
      { id: `PAY-${700 + payrollLogs.length + 1}`, employee: newName, basic: 45000, allowance: 4000, deductions: 2000, net: 47000, status: "Pending" }
    ]);
    setPerformanceRecords(prev => [
      ...prev,
      { id: `PRF-${800 + performanceRecords.length + 1}`, employee: newName, rating: 4.0, publications: 0, feedbackScore: "Good", targetAchieved: "85%" }
    ]);
    ...
    <div className="space-y-6">
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredEmployees.map(e => (
                    <tr key={e.id} className="hover:bg-slate-50/50 transition">
                      ...
                      <td className="py-3"><Badge tone="info">{e.dept}</Badge></td>
                      <td className="py-3 text-slate-600 font-semibold">{e.designation}</td>
                      <td className="py-3 text-center font-bold text-indigo-600">{e.leaveBalance} Days</td>
      ...
      {activeTab === "recruitment" && (
        <div className="grid lg:grid-cols-3 gap-6">
                ...
                <tbody className="divide-y divide-slate-100">
                  {recruitmentList.map(r => (
                    <tr key={r.id} className="hover:bg-slate-50/50 transition">
                      ...
                      <td className="py-3 text-center font-bold text-slate-700">{r.applicants} Candidates</td>
                      <td className="py-3 text-right">
          ...
          <Card>
            <h3 className="font-semibold text-slate-800 text-sm mb-4">Recruitment Pipeline Stats</h3>
            <div className="space-y-4">
              <div className="p-4 rounded-xl border bg-gradient-soft space-y-2">
                <div className="text-[10px] uppercase font-bold text-slate-400">Total Applicants Reviewable</div>
                <div className="text-2xl font-black text-indigo-600">47 Candidates</div>
                <div className="text-[10px] text-muted-foreground">Across all engineering and science open slots</div>
              </div>
              <div className="p-4 rounded-xl border bg-gradient-soft space-y-2">
                <div className="text-[10px] uppercase font-bold text-slate-400">Recruitment Phase</div>
        ...
        <Card>
          <div className="flex justify-between items-center mb-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Real-time status logs of classroom gateways & department portals.</p>
            </div>
            <button
              ...
              <tbody className="divide-y divide-slate-100">
                {biometricLogs.map(log => (
                  <tr key={log.id} className="hover:bg-slate-50/50 transition">
                    ...
                    <td className="py-3 text-right">
                      <Badge tone={log.status === "Late" ? "warn" : "success"}>
                        {log.status}
                      </Badge>
                    </td>
                  </tr>
      ...
      {activeTab === "leave" && (
        <div className="grid lg:grid-cols-3 gap-6">
          <Card className="lg:col-span-2">
            <h3 className="font-semibold text-slate-800 text-sm mb-4">Pending Leave Approval Requests</h3>
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b text-slate-400">
                ...
                <tbody className="divide-y divide-slate-100">
                  {leaveRequests.map(req => (
                    <tr key={req.id} className="hover:bg-slate-50/50 transition">
                      ...
                      <td className="py-3 text-center text-slate-500">{req.start} to {req.end}</td>
                      <td className="py-3 text-center font-bold text-indigo-600">{req.days} Days</td>
                      <td className="py-3 text-center">
                        <Badge tone={req.status === "Approved" ? "success" : req.status === "Rejected" ? "danger" : "warn"}>
                          {req.status}
                        </Badge>
                      </td>
                      <td className="py-3 text-right">
                        {req.status === "Pending" ? (
                          ...
                          <span className="text-slate-400 font-semibold italic text-[10px]">Processed</span>
                        )}
                      </td>
                    </tr>
                  ))}
        ...
        <Card>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Approve basic scales, allowances, deductions and credit salaries.</p>
            </div>
            <button
              ...
              <tbody className="divide-y divide-slate-100">
                {payrollLogs.map(p => (
                  <tr key={p.id} className="hover:bg-slate-50/50 transition">
                    ...
                    <td className="py-3 text-right font-mono font-semibold">₹{p.basic.toLocaleString()}</td>
                    <td className="py-3 text-right font-mono text-emerald-600 font-semibold">+₹{p.allowance.toLocaleString()}</td>
                    <td className="py-3 text-right font-mono text-rose-500 font-semibold">-₹{p.deductions.toLocaleString()}</td>
                    <td className="py-3 text-right font-mono font-bold text-indigo-700">₹{p.net.toLocaleString()}</td>
                    <td className="py-3 text-center">
                      <Badge tone={p.status === "Disbursed" ? "success" : "warn"}>
                        {p.status}
                      </Badge>
                    </td>
                    <td className="py-3 text-right">
      ...
      {activeTab === "performance" && (
        <Card>
          <div className="flex justify-between items-center mb-4">
            <div>
              <h3 className="font-semibold text-slate-800 text-sm">Faculty Performance Appraisals &amp; Publications</h3>
              <p className="text-xs text-muted-foreground mt-0.5">Audit key parameters for NAAC criteria (research publication and student feedback rating).</p>
            </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {performanceRecords.map(r => (
                  <tr key={r.id} className="hover:bg-slate-50/50 transition">
                    ...
                    <td className="py-3 text-center font-semibold text-slate-700">{r.targetAchieved}</td>
                    <td className="py-3 text-center font-bold text-slate-800">{r.publications} Papers</td>
                    <td className="py-3 text-right">
                      <Badge tone={r.feedbackScore === "Outstanding" || r.feedbackScore === "Excellent" ? "success" : "info"}>
                        {r.feedbackScore}
                      </Badge>
                    </td>
                  </tr>
