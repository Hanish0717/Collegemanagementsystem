          ...
          <GlassCard className="p-6 h-full flex flex-col">
            ...
            <div className="flex-1 space-y-3">
              {['Monthly Registration Summary', 'Q3 Placement Statistics', 'Annual Fundraising Report', 'Mentorship Engagement Analytics'].map((report, i) => (
                <div key={i} className="p-3 rounded-xl border bg-muted/30 hover:bg-muted/50 transition-colors flex items-center justify-between cursor-pointer group">
                  <span className="text-sm font-medium group-hover:text-primary transition-colors">{report}</span>
                  <Download className="w-4 h-4 text-muted-foreground group-hover:text-primary" />
                </div>
              ))}
            </div>
            <Button className="w-full mt-6 rounded-xl" variant="outline">Generate Custom Report</Button>
          </GlassCard>
        </div>
      </div>
    </div>
