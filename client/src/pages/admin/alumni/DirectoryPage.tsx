...
export function DirectoryPage() {
  ...
  const filtered = (directoryList || []).filter(
    (a: any) =>
  ...
  return (
    <div className="p-6 md:p-8 space-y-8 max-w-[1600px] mx-auto pb-24">
      ...
      <GlassCard className="p-6">
        ...
        <StyledTable
          headers={['Alumni', 'Batch / Dept', 'Current Role', 'Location', 'Status', 'Actions']}
        >
          {paginated.length > 0 ? (
            paginated.map((alumni: any, i: number) => (
              <TableRow key={i}>
                <TableCell>
                  <div className="flex items-center gap-3">
                    <img
                      src={
                        alumni.image ||
                        `https://api.dicebear.com/7.x/initials/svg?seed=${alumni.name}`
                      }
                      alt={alumni.name}
                ...
                <TableCell>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {alumni.department || 'Computer Science'}
                  </p>
                </TableCell>
                <TableCell>
