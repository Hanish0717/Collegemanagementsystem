...
export function PlacementsPage() {
  ...
  return (
    <div className="p-6 md:p-8 space-y-8 max-w-[1600px] mx-auto pb-24">
      ...
      >
        <Button className="rounded-xl bg-white text-blue-700 hover:bg-white/90">
          Add New Drive
        </Button>
      </GradientHeader>
        ...
        {activeTab === 'drives' && (
          <div className="space-y-6 animate-in fade-in">
            <AdvancedTableToolbar
              searchPlaceholder="Search placement drives..."
              onFilter={() => {}}
            />

            <StyledTable
              headers={['Company', 'Role', 'Date', 'Eligibility', 'Package', 'Status', 'Action']}
            >
              {upcomingDrives.map((drive) => (
                <TableRow key={drive.id}>
                  ...
                  <TableCell>
                    <Badge
                      variant={drive.status.includes('Open') ? 'default' : 'secondary'}
                  ...
                  <TableCell>
                    <Button
                      variant="ghost"
                      className="rounded-xl font-medium text-primary hover:bg-primary/5"
                    >
