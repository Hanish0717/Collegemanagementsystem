...
import { BookOpen, GraduationCap, MapPin, Clock, DollarSign, Search, ChevronRight, CheckCircle2 } from "lucide-react";
...
export function InternshipsPage() {
  ...
  const internships = [
    { id: 1, title: "Software Engineering Intern", company: "Tesla", stipend: "$8,000/mo", duration: "12 Weeks", mode: "On-site", location: "Austin, TX", skills: ["C++", "Python", "Robotics"], deadline: "2024-04-15", match: 92 },
    { id: 2, title: "Product Design Intern", company: "Airbnb", stipend: "$7,500/mo", duration: "10 Weeks", mode: "Remote", location: "Anywhere", skills: ["Figma", "UI/UX", "Prototyping"], deadline: "2024-04-20", match: 85 },
    { id: 3, title: "Data Science Intern", company: "Spotify", stipend: "$8,200/mo", duration: "14 Weeks", mode: "Hybrid", location: "New York, NY", skills: ["SQL", "Python", "Machine Learning"], deadline: "2024-04-10", match: 78 }
  ];
  ...
  return (
    <div className="p-6 md:p-8 space-y-8 max-w-[1600px] mx-auto pb-24">
      ...
      >
        <Button className="rounded-xl bg-white text-rose-600 hover:bg-white/90">Upload Resume</Button>
      </GradientHeader>
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-between relative">
          ...
          {[
            ...
            <div key={i} className="relative flex flex-col items-center gap-2 bg-card px-4 z-10">
              ...
              <span className={`text-xs font-semibold ${step.status === 'pending' ? 'text-muted-foreground' : 'text-foreground'}`}>{step.label}</span>
            </div>
          ))}
        </div>
      </GlassCard>
        ...
        {internships.map(internship => (
          <GlassCard key={internship.id} className="flex flex-col h-full hover:shadow-lg transition-all duration-300 border-transparent hover:border-rose-500/30">
            <div className="p-6 flex-1">
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex flex-col items-end gap-2">
                  <Badge variant="outline" className="text-emerald-600 bg-emerald-50 border-emerald-200">{internship.match}% Match</Badge>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-medium flex items-center"><DollarSign className="w-4 h-4 mr-1 text-muted-foreground"/> {internship.stipend}</p>
                </div>
                <div className="space-y-1">
            ...
            <div className="p-4 border-t bg-muted/10 flex items-center justify-between">
              <Button variant="ghost" className="rounded-xl">Details</Button>
              <Button className="rounded-xl bg-foreground text-background hover:bg-foreground/90">Apply Now <ChevronRight className="w-4 h-4 ml-1" /></Button>
            </div>
          </GlassCard>
        ))}
      </div>
