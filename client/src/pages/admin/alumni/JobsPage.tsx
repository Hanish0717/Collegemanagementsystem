      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <GlassCard className="p-6 bg-gradient-to-br from-purple-500/10 to-indigo-500/10 border-indigo-500/20">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              You have 3 active applications in progress.
            </p>
        ...
        <div className="lg:col-span-3 space-y-6">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <Button className="py-6 px-8 rounded-2xl bg-foreground text-background hover:bg-foreground/90">
              Search
            </Button>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {mockJobs.slice(0, 2).map((job: any) => (
                  <GlassCard key={job.id} glow className="border-indigo-500/30">
                    <div className="absolute top-0 right-0 p-1.5 px-3 bg-indigo-500 rounded-bl-xl text-xs font-bold text-white tracking-widest uppercase">
                      Featured
                    </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                Sort by:{' '}
                <Button variant="ghost" size="sm" className="font-semibold text-foreground px-2">
                  Most Recent <ChevronDown className="w-4 h-4 ml-1" />
                </Button>
              </div>
            </div>
            ...
            <div className="space-y-4">
              {filteredJobs.length > 0 ? (
                filteredJobs.map((job: any) => (
                  <GlassCard
                    key={job.id}
                    className="p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 hover:-translate-y-0.5 transition-transform"
                  >
                    <div className="flex items-center gap-5">
                      ...
                      <div>
                        <h4 className="text-lg font-bold hover:text-primary cursor-pointer transition-colors">
                          {job.title}
                        </h4>
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto">
                      ...
                      <Button className="rounded-xl flex-1 md:w-auto bg-gradient-to-r from-purple-600 to-indigo-600 hover:opacity-90">
                        Apply Now
                      </Button>
                    </div>
                  </GlassCard>
