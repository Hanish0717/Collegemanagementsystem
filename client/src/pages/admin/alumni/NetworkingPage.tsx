...
export function NetworkingPage() {
  ...
  const feed = [
    {
      ...
      author: {
        ...
        image: 'https://api.dicebear.com/7.x/initials/svg?seed=AJ',
      },
      time: '2 hours ago',
      content:
        'Just published a new article on advanced React patterns and performance optimization. Would love to hear thoughts from the alumni community!',
      likes: 124,
      comments: 18,
      liked: true,
    },
    {
      ...
      author: {
        ...
        image: 'https://api.dicebear.com/7.x/initials/svg?seed=SL',
      },
      time: '5 hours ago',
      content:
        'We are expanding our product team at Stripe! If any alumni are looking for roles in fintech, my DMs are open. #hiring #product',
      likes: 89,
      comments: 5,
      liked: false,
    },
  ...
  const suggestions = [
    ...
    },
    {
      ...
      mutual: 8,
      image: 'https://api.dicebear.com/7.x/initials/svg?seed=EW',
    },
    {
      name: 'James Wilson',
      title: 'Founder at TechStartup',
      mutual: 24,
      image: 'https://api.dicebear.com/7.x/initials/svg?seed=JW',
    },
  ];
          ...
          <GlassCard className="overflow-hidden">
            ...
            <div className="px-6 pb-6 relative text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1 mb-4">
                Class of 2020 • Software Engineer
              </p>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1 sm:gap-2">
                    <Button
                      variant="ghost"
                      size="sm"
                      className="text-muted-foreground hover:text-blue-600 rounded-lg"
                    >
            ...
            <GlassCard key={post.id} className="p-4 sm:p-6">
              <div className="flex items-start justify-between mb-4">
                <div className="flex gap-3">
                  ...
                  />
                  <div>
                    <h4 className="font-bold text-foreground hover:text-blue-600 cursor-pointer transition-colors leading-tight">
                      {post.author.name}
                    </h4>
              ...
              <div className="flex items-center justify-between gap-1 sm:gap-4 px-2">
                <Button
                  variant="ghost"
                  size="sm"
                  className={`flex-1 rounded-lg ${post.liked ? 'text-rose-500 hover:text-rose-600 hover:bg-rose-50' : 'text-muted-foreground hover:text-foreground hover:bg-muted/50'}`}
                >
                  <Heart className={`w-4 h-4 sm:mr-2 ${post.liked ? 'fill-current' : ''}`} />{' '}
                  <span className="hidden sm:inline">Like</span>
                </Button>
                <Button
                  variant="ghost"
                  size="sm"
                  className="flex-1 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted/50"
                >
          ...
          <GlassCard className="p-6">
            ...
            <div className="space-y-4">
              {suggestions.map((person, i) => (
                <div
                  key={i}
                  className="flex flex-col items-center text-center p-4 rounded-xl border bg-card/30 hover:bg-card transition-colors"
                >
                  <img src={person.image} className="w-16 h-16 rounded-full mb-3 bg-muted" />
                  <h4 className="font-bold text-sm leading-tight hover:text-blue-600 cursor-pointer">
                    {person.name}
                  </h4>
                  <p className="text-xs text-muted-foreground mt-1 mb-2 line-clamp-2 min-h-[32px]">
                    {person.title}
                  </p>
