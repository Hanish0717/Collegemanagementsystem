  ...
  const mentors = [
    {
      ...
      rating: 4.9,
      reviews: 42,
      skills: ['Machine Learning', 'Python', 'Ethics'],
    ...
    },
    {
      ...
      rating: 4.8,
      reviews: 28,
      skills: ['Leadership', 'System Design', 'Fintech'],
    ...
    <div className="p-6 md:p-8 space-y-8 max-w-[1600px] mx-auto pb-24">
      ...
      >
        <div className="flex gap-2">
          ...
          <Button className="rounded-xl bg-white text-orange-600 hover:bg-white/90">
            My Sessions
          </Button>
        </div>
      </GradientHeader>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <GlassCard className="p-6 bg-gradient-to-br from-amber-500/10 to-orange-500/10 border-orange-500/20">
            <h3 className="font-bold text-lg mb-4 flex items-center gap-2">
              <CalendarIcon className="w-5 h-5 text-orange-600" /> Next Session
            </h3>
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <Button
              variant="outline"
              className="py-6 px-6 rounded-2xl bg-card border-none shadow-sm text-foreground"
            >
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2">
            {['All', 'Engineering', 'Product', 'Design', 'Data Science', 'Marketing'].map((cat) => (
              <Badge
                ...
                className="px-4 py-1.5 rounded-lg text-sm cursor-pointer whitespace-nowrap hover:bg-primary/90 hover:text-primary-foreground transition-colors"
              >
                {cat}
              </Badge>
            ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {mentors.map((mentor) => (
                <MentorCard
                  key={mentor.id}
                  mentor={mentor}
                  onRequest={() => console.log('Book session with', mentor.name)}
                />
              ))}
            </div>
          </div>
