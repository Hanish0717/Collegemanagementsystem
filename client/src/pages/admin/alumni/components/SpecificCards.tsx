...
import { MapPin, Briefcase, Calendar, DollarSign, ExternalLink, ArrowRight, Clock, Star, Users } from 'lucide-react';
  ...
  return (
    <GlassCard className="p-6 flex flex-col h-full hover:-translate-y-1 transition-transform duration-300">
      ...
      <div className="flex items-center justify-between mt-auto">
        ...
        <Button size="sm" onClick={onApply} className="rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:opacity-90 transition-opacity">Apply Now</Button>
      </div>
    </GlassCard>
  );
}
  ...
  return (
    <div className="group rounded-3xl border bg-card p-6 flex flex-col h-full hover:shadow-lg transition-all duration-300">
      <div className="flex items-center gap-4 mb-4">
        <img src={mentor.image || `https://api.dicebear.com/7.x/initials/svg?seed=${mentor.name}`} alt={mentor.name} className="w-16 h-16 rounded-full object-cover border-2 border-primary/20" />
        <div>
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {(mentor.skills?.length || 0) > 3 && (
            <Badge variant="outline" className="text-[10px] py-0">+{mentor.skills.length - 3}</Badge>
          )}
        </div>
      </div>

      <p className="text-sm text-muted-foreground line-clamp-2 mb-6 flex-1">
        {mentor.bio || "Passionate about helping the next generation of engineers build great products."}
      </p>

      <div className="flex items-center gap-2 mt-auto">
        <Button variant="outline" className="flex-1 rounded-xl">View Profile</Button>
        <Button onClick={onRequest} className="flex-1 rounded-xl bg-gradient-to-r from-violet-600 to-purple-600">Connect</Button>
      </div>
    </div>
  );
}
...
export function EventCard({ event, onRegister }: { event: any, onRegister?: () => void }) {
  ...
  return (
    <div className={cn("group rounded-3xl border overflow-hidden flex flex-col h-full transition-all duration-300", isPast ? "opacity-70 bg-muted/30" : "bg-card hover:shadow-xl")}>
      <div className="relative h-48 overflow-hidden">
        <div className={cn("absolute inset-0 bg-gradient-to-t from-black/80 to-transparent z-10")} />
        <img 
        ...
        <div className="absolute top-4 right-4 z-20">
          <Badge className={cn("backdrop-blur-md", isPast ? "bg-black/50 text-white" : "bg-primary/90 text-primary-foreground")}>
        ...
        <div className="absolute bottom-4 left-4 z-20 text-white">
          ...
          <p className="text-xs text-white/80 font-medium flex items-center gap-1.5">
            <Calendar className="w-3 h-3" /> {new Date(event.date).toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'short', day: 'numeric' })}
          </p>
        </div>
      </div>
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="mt-auto pt-4 border-t flex items-center justify-between">
          ...
          <Button 
            ...
            onClick={onRegister}
            size="sm" 
            className={cn("rounded-xl", !isPast && "bg-gradient-to-r from-rose-500 to-orange-500 hover:from-rose-600 hover:to-orange-600")}
          >
