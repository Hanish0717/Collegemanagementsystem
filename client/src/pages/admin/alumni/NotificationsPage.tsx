  ...
  const notifications = [
    {
      ...
      text: 'Your alumni profile has been verified.',
      time: '10 mins ago',
      unread: true,
      icon: ShieldAlert,
    ...
    },
    {
      ...
      text: 'David Chen sent you a connection request.',
      time: '2 hours ago',
      unread: true,
      icon: Heart,
      ...
      <GlassCard className="overflow-hidden">
        <div className="divide-y divide-border/50">
          {notifications.map((n) => (
            <div
              key={n.id}
              className={`p-6 flex gap-4 transition-colors hover:bg-muted/30 cursor-pointer ${n.unread ? 'bg-primary/5' : ''}`}
            >
              <div
                className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 ${n.color}`}
              >
              ...
              <div className="flex-1">
                <div className="flex justify-between items-start mb-1">
                  <h4
                    className={`text-base ${n.unread ? 'font-bold text-foreground' : 'font-semibold text-foreground/80'}`}
                  >
                    {n.title}
                  </h4>
