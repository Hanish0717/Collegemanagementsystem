        ...
        <div className="flex items-center justify-between relative">
          ...
          {steps.map((s) => (
            <div key={s.id} className="flex flex-col items-center gap-2 bg-background p-2">
              <div
                className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 ${step >= s.id ? 'bg-primary text-primary-foreground shadow-lg shadow-primary/30' : 'bg-muted text-muted-foreground border-2 border-background'}`}
              >
                <s.icon className="w-5 h-5" />
              </div>
              <span
                className={`text-xs font-semibold ${step >= s.id ? 'text-primary' : 'text-muted-foreground'}`}
              >
                {s.title}
              </span>
            </div>
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <FormGroup
                label="Email Address"
                required
                description="We'll send an OTP for verification"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <FormGroup label="Profile Photo" description="Upload a professional headshot">
                <FileUploadZone
                  ...
                  subLabel="JPG, PNG up to 2MB"
                />
              </FormGroup>
              <FormGroup
                label="ID Proof / Degree Certificate"
                required
                description="Required for admin verification"
              >
                <FileUploadZone
                  accept=".pdf,.jpg,.png"
                  label="Upload Document"
                  subLabel="PDF, JPG up to 5MB"
                />
              </FormGroup>
            </div>
          </div>
