    ...
    () =>
      adminUsers.filter(
        (admin) =>
          ...
          [admin.id, admin.name, admin.role, admin.department].some((value) =>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="border-b">
              <tr>
                {["Admin ID", "Name", "Role", "Department", "Status", "Last Login", "Actions"].map(
                  (column) => (
                    <th
                      key={column}
        ...
        <Card>
          ...
          <div className="space-y-4 p-4 border rounded-xl bg-gradient-soft">
            ...
            <div className="grid sm:grid-cols-3 gap-2">
              {["Students", "Faculty", "Fees", "Reports", "Settings", "Security"].map(
                (permission) => (
                  <label
                    key={permission}
                    className="flex items-center gap-2 p-2 rounded-lg border bg-background text-sm"
                  >
