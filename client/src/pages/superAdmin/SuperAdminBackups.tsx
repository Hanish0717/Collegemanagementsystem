          ...
          <div className="space-y-3">
            {[
              ...
              >
                ...
                <button
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition ${index < 3 ? "bg-emerald-500" : "bg-muted"}`}
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition ${index < 3 ? "translate-x-6" : "translate-x-1"}`}
                  />
                </button>
              </div>
            ))}
        ...
        <div className="space-y-4 p-4 border rounded-xl bg-gradient-soft">
          <div className="grid sm:grid-cols-3 gap-4">
            <select className="rounded-lg border bg-background px-3 py-2 text-sm">
              {backups.map((backup) => (
                <option key={backup.id}>{backup.id}</option>
              ))}
            </select>
            <input
              defaultValue="Full restore"
              className="rounded-lg border bg-background px-3 py-2 text-sm"
            />
            <input
              defaultValue="Maintenance window"
              className="rounded-lg border bg-background px-3 py-2 text-sm"
            />
          </div>
