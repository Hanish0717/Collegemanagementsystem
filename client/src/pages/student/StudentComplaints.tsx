    ...
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-4 gap-4">
        {[
          ...
          { label: "Pending", value: history.filter(c => c.status === "Pending").length.toString(), tone: "warn" as const },
          { label: "Resolved", value: history.filter(c => c.status === "Resolved").length.toString(), tone: "success" as const },
          { label: "In Progress", value: history.filter(c => c.status === "In Progress").length.toString(), tone: "info" as const },
        ].map(stat => (
          <Card key={stat.label}>
        ...
        <Card>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 p-4 border rounded-xl bg-gradient-soft">
            <div>
              ...
              >
                {["Infrastructure", "Academic", "Hostel", "Canteen", "Other"].map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div>
            ...
            <button type="submit" disabled={loading} className="w-full px-4 py-2.5 rounded-lg bg-gradient-primary text-white text-sm font-medium flex items-center justify-center gap-2">
