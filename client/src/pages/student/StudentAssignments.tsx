    ...
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-4 gap-4">
        {[
          ...
          { label: "Submitted", value: submittedList.length.toString(), tone: "success" as const },
          { label: "Overdue", value: list.filter(a => !a.submitted && new Date(a.dueDate) < new Date()).length.toString(), tone: "danger" as const },
        ].map(stat => (
          <Card key={stat.label}>
          ...
          {list.map(assignment => (
            <Card key={assignment.id} className="hover:-translate-y-1 transition">
              <div className="flex items-start justify-between mb-4">
                ...
                <Badge tone={assignment.status === "Submitted" || assignment.status === "Graded" ? "success" : "warn"}>{assignment.status}</Badge>
              </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-4 border rounded-xl bg-gradient-soft">
          <div>
            ...
            >
              ...
              {pendingList.map(a => <option key={a.id} value={a.id}>{a.title} ({a.subject})</option>)}
            </select>
          </div>
          <div>
            ...
            submittedList.map(assignment => (
              <div key={assignment.id} className="flex items-center gap-3 p-3 rounded-xl border hover:bg-accent/50 transition">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-muted-foreground">{assignment.subject} • Status: {assignment.status}</div>
                </div>
