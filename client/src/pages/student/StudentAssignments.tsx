    ...
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-4 gap-4">
        {[
          ...
          { label: "Pending", value: assignments.filter(a => !a.submitted).length.toString(), tone: "warn" as const },
          { label: "Submitted", value: assignments.filter(a => a.submitted).length.toString(), tone: "success" as const },
          ...
          {assignments.map(assignment => (
            <Card key={assignment.id} className="hover:-translate-y-1 transition">
              <div className="flex items-start justify-between mb-4">
                ...
                <Badge tone={assignment.status === "Submitted" ? "success" : "warn"}>{assignment.status}</Badge>
              </div>
      ...
      <Card>
        ...
        <div className="space-y-4 p-4 border rounded-xl bg-gradient-soft">
          <select className="w-full rounded-lg border bg-background px-3 py-2 text-sm">
            {assignments.filter(a => !a.submitted).map(a => <option key={a.id}>{a.title}</option>)}
          </select>
          <textarea placeholder="Assignment description or notes..." rows={3} className="w-full rounded-lg border bg-background px-3 py-2 text-sm" />
      ...
      <Card>
        ...
        <div className="space-y-2">
          {assignments.filter(a => a.submitted).map(assignment => (
            <div key={assignment.id} className="flex items-center gap-3 p-3 rounded-xl border hover:bg-accent/50 transition">
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-muted-foreground">{assignment.subject} • Submitted on {assignment.dueDate}</div>
              </div>
