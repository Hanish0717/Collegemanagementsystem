        ...
        >
                  ...
                  <div>
                    ...
                    <div className="mt-1 relative">
                      ...
                      <input
                        ...
                        className="w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50 pl-10 pr-11 py-2.5 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 transition"
                      />
                      <button
                        type="button"
                        onClick={() => setShowPassword(!showPassword)}
                        className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition cursor-pointer"
          ...
          {(() => {
            ...
            return (
              <button
                type="button"
                onClick={() => (hasGoogleClientId ? googleLogin() : null)}
                disabled={googleLoading || !hasGoogleClientId}
                className={`mt-4 w-full flex items-center justify-center gap-3 rounded-xl border py-2.5 text-xs font-bold transition-all ${
                  hasGoogleClientId
                    ? "border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50 text-slate-700 dark:text-slate-200 hover:bg-slate-100 cursor-pointer"
                    : "border-slate-200/60 dark:border-slate-800/60 bg-slate-100/50 dark:bg-slate-900/50 text-slate-400 dark:text-slate-500 cursor-not-allowed opacity-75"
                }`}
              >
                {googleLoading ? (
                  <Loader2 className="size-4 animate-spin" />
                ) : (
                  <svg className="size-4 opacity-80" viewBox="0 0 24 24">
                    <path
                      fill="#4285F4"
