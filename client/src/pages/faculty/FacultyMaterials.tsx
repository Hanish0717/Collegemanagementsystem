...
export function FacultyMaterials() {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    const finalUrl =
      fileUrl ||
      'https://example.com/materials/doc-' + Date.now() + (type === 'PDF' ? '.pdf' : '.mp4');
    setLoading(true);
    try {
    ...
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-4 gap-4">
        {[
          ...
          {
            label: 'PDF Documents',
            value: list.filter((m) => m.type === 'PDF').length.toString(),
            tone: 'info' as const,
          },
          {
            label: 'Videos',
            value: list.filter((m) => m.type === 'Video').length.toString(),
            tone: 'info' as const,
          },
          {
            label: 'Total Downloads',
            value: list.reduce((sum, m) => sum + m.downloads, 0).toLocaleString(),
