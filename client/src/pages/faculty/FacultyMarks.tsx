    ...
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-4 gap-4">
        {[
          ...
          {
            label: 'Submitted',
            value: marks.filter((m) => m.status === 'Submitted').length.toString(),
            tone: 'success' as const,
          },
          {
            label: 'Pending',
            value: marks.filter((m) => m.status === 'Pending').length.toString(),
            tone: 'warn' as const,
          },
          {
            label: 'Average Score',
            value:
              marks.length > 0
                ? `${Math.round(marks.reduce((sum, m) => sum + Number(m.marks), 0) / marks.length)}%`
                : '0%',
            tone: 'success' as const,
          },
        ].map((stat) => (
