...
export function HodDashboard() {
  const [facultyRoster, setFacultyRoster] = useState([
    { name: "Dr. Srinivas Rao", designation: "Professor", workload: 16, subject: "Machine Learning" },
    { name: "Mrs. Ananya Sen", designation: "Assistant Professor", workload: 14, subject: "Compiler Design" },
    { name: "Mr. Ramesh Yadav", designation: "Senior Lecturer", workload: 18, subject: "Database Management" },
    { name: "Dr. K. Lakshmi", designation: "Associate Professor", workload: 12, subject: "Data Structures" }
  ]);
  ...
  const [leaves, setLeaves] = useState([
    { id: "L-802", faculty: "Mrs. Ananya Sen", days: 3, dates: "July 20-22", reason: "Family Event" },
    { id: "L-810", faculty: "Dr. Srinivas Rao", days: 1, dates: "July 25", reason: "Medical Checkup" }
  ]);
  ...
  const handleEmailAllDefaulters = () => {
    toast.success(`Automated attendance warning emails dispatched to ${warnings.length} student portals.`);
  };
      ...
      <div className="grid lg:grid-cols-3 gap-4">
              ...
              <tbody className="divide-y divide-slate-100">
                {facultyRoster.map(f => (
                  <tr key={f.name} className="hover:bg-slate-50">
                    ...
                    <td className="py-2.5 text-center font-bold text-indigo-700">{f.workload} hrs</td>
        ...
        <Card>
          ...
          <div className="space-y-3">
            {leaves.length === 0 ? (
              <div className="text-xs text-muted-foreground text-center py-6">All department leave requests processed.</div>
            ) : (
              leaves.map(l => (
                <div key={l.id} className="p-3 border rounded-xl space-y-2 text-xs bg-slate-50/50 hover:bg-slate-50 transition">
                  ...
                  <p className="text-slate-500 font-medium">Reason: {l.reason} ({l.days} Days)</p>
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-semibold text-slate-800 text-sm flex items-center gap-1.5">
                <AlertTriangle className="size-4 text-amber-500" /> Attendance Warning Board (&lt;75% Attendance)
              </h3>
              <p className="text-[10px] text-slate-500">Auto-calculated from biometric attendance databases.</p>
            </div>
            <button
            ...
            {warnings.map(w => (
              <div key={w.id} className="p-3 border rounded-xl flex items-center justify-between text-xs hover:bg-slate-50/50 transition">
                <div>
                  ...
                  <div className="text-slate-400 text-[10px] mt-0.5">Roll No: {w.roll} • Sec: {w.section}</div>
                </div>
        ...
        <Card className="flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              Post instructions to CSE student portals, schedule faculty staff meetings, or modify course syllabi notifications.
            </p>
          </div>
          <div className="space-y-2 mt-4">
            ...
            <button
              onClick={() => {
                toast.success("Automated notifications generated for NBA Criteria 2 Course Files audits.");
              }}
