      ...
      <div className="grid md:grid-cols-4 gap-4">
        <Card className="bg-background border border-slate-200/80 dark:border-slate-800 rounded-2xl p-4 flex flex-col justify-between">
          <div>
            ...
            <div className="text-3xl font-extrabold mt-2 text-slate-800 dark:text-slate-100">
              {stats.total}
            </div>
          </div>
        ...
        <Card className="bg-background border border-slate-200/80 dark:border-slate-800 rounded-2xl p-4 flex flex-col justify-between">
          <div>
            ...
            <div className="text-3xl font-extrabold mt-2 text-indigo-650 dark:text-indigo-400">
              +{stats.newThisWeek}
            </div>
          </div>
      ...
      {isResidentsLoading ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredResidents.map((r: any) => {
            const initials = r.resident_name
              ? r.resident_name
                  .split(' ')
                  .map((n: string) => n[0])
                  .join('')
              ...
              >
                ...
                <div className="p-4 bg-gradient-to-br from-indigo-50/40 via-slate-50/20 to-transparent dark:from-indigo-950/15 dark:via-transparent pb-2 flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-extrabold text-sm text-slate-800 dark:text-slate-100 truncate max-w-40">
                        {r.resident_name}
                      </h4>
                      <span className="text-[9px] text-slate-400 tracking-wider font-mono uppercase block mt-0.5">
                        ID: {r.resident_id?.substring(0, 10)}
                      </span>
                    </div>
                  </div>

                  <Badge tone="success" className="text-[9px] py-0.5 px-2">
                    Subscribed
                  </Badge>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-x-2 gap-y-1">
                    <div>
                      ...
                      <span className="font-bold text-slate-700 dark:text-slate-300">
                        Room {r.room_number || 'Unallocated'}
                      </span>
                    </div>
                    <div>
                      <span className="text-[9px] text-slate-400 block uppercase tracking-wider">
                ...
                <div className="p-4 pt-2 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between gap-4">
                  ...
                  <div className="flex flex-col gap-0.5 opacity-40 group-hover:opacity-70 transition-opacity">
                    ...
                    <span className="text-[8px] font-mono tracking-widest text-slate-500 uppercase">
                      MEM-{r.id?.substring(0, 6)}
                    </span>
                  </div>

                  <button
                    onClick={() => {
                      if (!confirm(`Cancel mess membership subscription for ${r.resident_name}?`))
                        return;
                      ...
                      >
                        <div>
                          ...
                          <div className="text-[10px] text-muted-foreground">
                            {hr.rollNumber} • Room {hr.roomNumber}
                          </div>
                        </div>
              ...
              {selectedStudentId && (
                <div className="p-4 bg-slate-50 dark:bg-slate-900/60 border rounded-2xl space-y-2 text-xs animate-in fade-in duration-300">
                  ...
                  <div className="flex justify-between border-b pb-1.5">
                    ...
                    <strong className="text-slate-755 dark:text-slate-200">
                      Block {form.hostel_block || '—'}
                    </strong>
                  </div>
