...
export function AccountsDashboard() {
  const [vendorInvoices, setVendorInvoices] = useState([
    { id: "INV-801", vendor: "Sai Security Services", category: "Campus Guard Wages", amount: 120000, due: "July 25", status: "Pending" },
    { id: "INV-802", vendor: "Nirmal Caterers", category: "Hostel Mess Provisions", amount: 185000, due: "July 28", status: "Pending" },
    { id: "INV-803", vendor: "Supreme Books Dist", category: "Library Catalog Purchases", amount: 65000, due: "August 02", status: "Pending" }
  ]);
  ...
  const handleRunPayroll = () => {
    toast.loading("Processing salary schedules, bank files, and TDS deductions...", { duration: 1500 });
    setTimeout(() => {
    ...
    <div className="space-y-6">
        ...
        <Card className="lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-[10px] text-slate-500">Collected amount vs outstanding pending dues in Lakhs (₹)</p>
            </div>
          ...
          <div className="h-64">
            <ResponsiveContainer>
              <BarChart data={feeSourceData}>
                ...
                <Tooltip />
                <Bar dataKey="collected" name="Collected (Lakhs)" fill="#10B981" radius={[6, 6, 0, 0]} />
                <Bar dataKey="pending" name="Pending (Lakhs)" fill="#F59E0B" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </Card>
        ...
        <Card>
          ...
          <div className="space-y-3">
            {vendorInvoices.length === 0 ? (
              <div className="text-xs text-muted-foreground text-center py-6">All vendor bills paid!</div>
            ) : (
              vendorInvoices.map(row => (
                <div key={row.id} className="p-3 border rounded-xl space-y-2 text-xs hover:bg-slate-50/50 transition">
                  <div className="flex justify-between font-bold text-slate-800">
                    <span>{row.vendor}</span>
                    <span className="font-mono text-indigo-600">₹{row.amount.toLocaleString()}</span>
                  </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        <Card className="lg:col-span-2 flex flex-col justify-between">
          <div>
            <h3 className="font-semibold text-slate-800 text-sm mb-1.5">Staff Salaries &amp; Payroll Run</h3>
            <p className="text-xs text-muted-foreground mb-4">
              Disburse monthly compensation to teaching and non-teaching personnel, calculate tax withholdings, and schedule direct deposits.
            </p>
        ...
        <Card className="flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              Verify institutional ledgers, export tax files, review scholarship distributions, and audit GST filings.
            </p>
          </div>
