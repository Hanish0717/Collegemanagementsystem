...
export function HostelReports() {
  const [activeReport, setActiveReport] = useState<
    'residents' | 'occupancy' | 'block' | 'fees' | 'available-rooms'
  >('residents');
  ...
  const { data: residentData, isLoading: residentLoading } = useQuery({
    ...
    queryFn: () =>
      fetchResidentReport({ ...filters, search: searchTerm, sortBy, order: sortOrder }),
    staleTime: 1000 * 60 * 5,
  });
  ...
  const handleExport = useCallback(
    async (format: 'pdf' | 'excel', type: string) => {
      ...
      try {
        if (type === 'residents') {
          ...
          format === 'pdf'
            ? await exportOccupancyPDF(filters)
            : await exportOccupancyExcel(filters);
        } else if (type === 'fees') {
  ...
  const blockChartData = useMemo(() => {
    ...
    return blockData.data.map((b: any) => ({
      name: b.blockName,
      beds: b.capacity,
      occupied: b.occupiedBeds,
      available: b.availableBeds,
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Hostel Reports & Analytics"
        desc="Reports, exports, and dashboard metrics"
      />
      ...
      {!analyticsLoading && analyticsData && (
        <div className="grid md:grid-cols-4 gap-4">
          ...
          <Card>
            <div className="text-xs text-muted-foreground flex items-center gap-1">
              <TrendingUp className="size-4" /> Occupancy Rate
            </div>
        ...
        <div className="flex flex-col lg:flex-row gap-4 items-center justify-between mb-6">
          <div className="flex gap-2 flex-wrap">
            {[
              ...
              <button
                ...
                className={`px-4 py-2.5 rounded-lg text-sm font-medium transition ${activeReport === tab.id ? 'bg-primary text-white' : 'border border-slate-200 dark:border-slate-700 hover:bg-accent'}`}
              >
                {tab.label}
              </button>
            ))}
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 text-sm hover:bg-accent transition flex items-center gap-2"
            >
            ...
            <div className="relative group">
              <button className="px-4 py-2.5 rounded-lg bg-primary text-white text-sm hover:opacity-95 transition flex items-center gap-2">
                <FileDown className="size-4" /> Export <ChevronDown className="size-4" />
              </button>
              <div className="absolute right-0 mt-2 w-48 bg-background border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg hidden group-hover:block z-10">
                <button
                  onClick={() => handleExport('pdf', activeReport)}
                  disabled={exporting === `${activeReport}-pdf`}
                  className="block w-full text-left px-4 py-2 hover:bg-accent text-sm transition disabled:opacity-50"
                >
                  {exporting === `${activeReport}-pdf` ? <>Exporting...</> : 'Export as PDF'}
                </button>
                <button
                  onClick={() => handleExport('excel', activeReport)}
                  disabled={exporting === `${activeReport}-excel`}
                  className="block w-full text-left px-4 py-2 hover:bg-accent text-sm transition disabled:opacity-50"
                >
                  {exporting === `${activeReport}-excel` ? <>Exporting...</> : 'Export as Excel'}
                </button>
              </div>
            </div>
          </div>
            ...
            <div className="grid md:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFilters({ ...filters, roomType: e.target.value || undefined })
                  }
                  className="w-full mt-1 px-3 py-2 rounded-lg border"
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFilters({ ...filters, acType: e.target.value || undefined })}
                  className="w-full mt-1 px-3 py-2 rounded-lg border"
                >
          ...
          <div>
            ...
            {residentLoading ? (
              ...
              <div className="flex items-center justify-center py-8 text-sm text-muted-foreground">
                <AlertCircle className="mr-2" /> No residents found
              </div>
            )}
          </div>
        )}
                ...
                <div className="grid md:grid-cols-4 gap-4 mb-4">
                  ...
                  <Card>
                    ...
                    <div className="text-2xl font-bold mt-2 text-emerald-600">
                      {occupancyData?.summary?.totalAvailableBeds}
                    </div>
                  </Card>
                  <Card>
                    ...
                    <div className="text-2xl font-bold mt-2 text-indigo-600">
                      {occupancyData?.summary?.totalBeds
                        ? Math.round(
                            (occupancyData.summary.totalOccupiedBeds /
                              occupancyData.summary.totalBeds) *
                              100,
                          )
                        : 0}
              ...
              <>
                <div className="grid md:grid-cols-4 gap-4 mb-4">
                  <Card>
                    ...
                    <div className="text-2xl font-bold mt-2">{feeData?.summary?.totalRecords}</div>
                  </Card>
                  <Card>
                    <div className="text-xs text-muted-foreground">Total Amount</div>
                    <div className="text-2xl font-bold mt-2">
                      {formatCurrency(feeData?.summary?.totalAmount || 0)}
                    </div>
                  </Card>
                  <Card>
                  ...
                  <Card>
                    ...
                    <div className="text-2xl font-bold mt-2 text-amber-600">
                      {formatCurrency(feeData?.summary?.pendingAmount || 0)}
                    </div>
                  </Card>
                </div>
                        ...
                        <PieChart>
                          ...
                          >
                            {feeChartData.map((_, i) => (
                              <Cell key={i} fill={['#4F46E5', '#06B6D4', '#10B981'][i % 3]} />
                            ))}
                          </Pie>
                    ...
                    <table className="w-full text-sm">
                      <thead className="border-b">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-left">AC Rooms</th>
                      ...
                      <tbody>
                        {blockData?.data?.map((b: any, i: number) => (
                          <tr key={i} className="border-b hover:bg-muted/30">
                            <td className="px-4 py-3 font-medium">{b.blockName}</td>
                            <td className="px-4 py-3">{b.totalRooms}</td>
                            <td className="px-4 py-3">{b.acRooms}</td>
                            <td className="px-4 py-3">{b.capacity}</td>
                            <td className="px-4 py-3 font-medium">{b.occupiedBeds}</td>
                            <td className="px-4 py-3 text-emerald-600">{b.availableBeds}</td>
                            <td className="px-4 py-3">
                              <Badge tone={b.occupancyRate >= 80 ? 'warn' : 'info'}>
                                {b.occupancyRate}%
                              </Badge>
                            </td>
                          </tr>
              ...
              <>
                <div className="grid md:grid-cols-3 gap-4 mb-4">
                  <Card>
                    ...
                    <div className="text-2xl font-bold mt-2">{availableRoomsData?.total}</div>
                  </Card>
                  <Card>
                    <div className="text-xs text-muted-foreground">Available Beds</div>
                    <div className="text-2xl font-bold mt-2 text-emerald-600">
                      {availableRoomsData?.data?.reduce(
                        (s: any, r: any) => s + r.availableBeds,
                        0,
                      ) || 0}
                    </div>
                  </Card>
                    ...
                    <table className="w-full text-sm">
                      <thead className="border-b">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-left">Available</th>
                          <th className="px-4 py-3 text-left">AC/Non-AC</th>
                        </tr>
                      </thead>
                      <tbody>
                        {availableRoomsData?.data?.map((r: any, i: number) => (
                          <tr key={i} className="border-b hover:bg-muted/30">
                            <td className="px-4 py-3 font-medium">{r.hostelBlock}</td>
                            <td className="px-4 py-3 font-semibold">{r.roomNumber}</td>
                            <td className="px-4 py-3">{r.roomType}</td>
                            <td className="px-4 py-3">{r.capacity}</td>
                            <td className="px-4 py-3">{r.currentOccupancy}</td>
                            <td className="px-4 py-3 text-emerald-600">{r.availableBeds}</td>
                            <td className="px-4 py-3">
                              <Badge tone={r.acType === 'AC' ? 'info' : 'warn'}>{r.acType}</Badge>
                            </td>
                          </tr>
                        ))}
                      </tbody>
