...
export default function AllocationsPage() {
  ...
  const { data: rooms = [] } = useQuery({
    ...
    queryFn: () => fetchHostelRooms(),
  });

  const transferMut = useMutation({
    mutationFn: ({ id, payload }: any) => transferAllocation(id, payload),
    onSuccess: () => {
  ...
  const deleteMut = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['hostel-allocations'] }),
  });
              ...
              <div key={a.id} className="flex justify-between items-center p-3 border rounded">
                <div>
                  <div className="font-medium">
                    {a.students?.full_name || a.student_name} —{' '}
                    {a.hostel_rooms?.room_number || a.room_number}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    Bed: {a.bed_number} • {a.academic_year}
                  </div>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => setTransferTarget(a)} className="px-2 py-1 border rounded">
                    Transfer
                  </button>
                  <button
        ...
        <div className="fixed inset-0 grid place-items-center bg-black/40 p-4">
          <div className="bg-background p-6 rounded-xl border max-w-md w-full">
            <h4 className="font-semibold mb-3">
              Transfer {transferTarget.students?.full_name || transferTarget.student_name}
            </h4>
            <form
              onSubmit={(e) => {
                e.preventDefault();
                ...
                const payload = {
                  newRoomId: form.room_id.value,
                  newBedNumber: Number(form.bed_number.value || 1),
                  newBlockId: form.block_id?.value,
                };
                ...
                <select name="room_id" className="w-full rounded border px-3 py-2">
                  {(rooms || []).map((r: any) => (
                    <option key={r.id} value={r.id}>
                      {r.room_number} — {r.hostel_blocks?.name || r.blockName}
                    </option>
                  ))}
                </select>
                <input
