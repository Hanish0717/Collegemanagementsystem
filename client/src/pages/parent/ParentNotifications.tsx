...
import { parentNotificationItems } from "@/mock/parentData";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-4 gap-4">
        {[
          { label: "Total Notifications", value: parentNotificationItems.length.toString(), tone: "info" as const },
      ...
      <Card>
        <div className="flex flex-wrap gap-2">
          {["All", "Unread", "High Priority", "Medium Priority", "Low Priority"].map((filter, index) => (
      ...
      <Card>
        ...
        <div className="space-y-2">
          {parentNotificationItems.map((notification, index) => (
            <div key={notification.title} className={`flex items-start gap-4 p-4 rounded-xl border hover:bg-accent/50 transition ${index < 2 ? "bg-blue-50 border-blue-200" : ""}`}>
              <div className={`size-2 rounded-full mt-2 shrink-0 ${index < 2 ? "bg-primary" : "bg-muted"}`} />
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  <div className="text-sm font-medium">{notification.title}</div>
                  <Badge tone={notification.priority === "High" ? "danger" : notification.priority === "Medium" ? "warn" : "info"}>
                    {notification.priority}
                  </Badge>
                </div>
                <div className="text-xs text-muted-foreground mt-1">{notification.type} • {notification.time}</div>
              </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <Card>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={setting.label} className="flex items-center justify-between p-3 rounded-xl border">
                <span className="text-sm">{setting.label}</span>
                <button className={`relative inline-flex h-6 w-11 items-center rounded-full transition ${setting.enabled ? "bg-emerald-500" : "bg-muted"}`}>
        ...
        <Card>
          ...
          <div className="space-y-2">
            {[
              { category: "Exam", count: 5, unread: 1 },
              { category: "Alert", count: 3, unread: 1 },
              { category: "Meeting", count: 2, unread: 0 },
              { category: "Event", count: 4, unread: 0 },
            ].map(item => (
              <div key={item.category} className="flex items-center justify-between p-3 rounded-xl border hover:bg-accent/50 transition cursor-pointer">
