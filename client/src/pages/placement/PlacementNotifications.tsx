...
import { placementNotifications } from "@/mock/mockData";

export function PlacementNotifications() {
  const [selectedNotif, setSelectedNotif] = useState<string | null>(null);
  const [archived, setArchived] = useState<string[]>([]);

  const unreadCount = placementNotifications.filter(
    (n) => n.unread && !archived.includes(n.id),
  ).length;
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <Card className="text-center">
          ...
          <div className="text-2xl font-bold">
            {Math.round((unreadCount / activeNotifications.length) * 100)}%
          </div>
        ...
        {activeNotifications.map((notif) => (
          ...
          >
            <div className="flex items-start gap-3">
              <div
                className={`size-12 rounded-lg bg-gradient-to-br ${typeColors[notif.type]} text-white grid place-items-center shrink-0`}
              >
                {notif.type === "Drive" && "🎯"}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {notif.unread && <div className="size-2.5 rounded-full bg-blue-600" />}
                ...
                {selectedNotif === notif.id && (
                  <div className="mt-3 p-3 bg-background rounded-lg border text-sm space-y-2">
                    <p className="text-muted-foreground">
                      {notif.type === "Drive" &&
                        "A new recruitment drive has been added. Check eligibility and apply before deadline."}
                      {notif.type === "Interview" &&
                        "Your interview has been scheduled. Please confirm availability and prepare accordingly."}
                      {notif.type === "Offer" &&
                        "Congratulations! You have received an offer. Review terms and accept within 5 business days."}
                      {notif.type === "Deadline" &&
                        "Application deadline is approaching. Submit your application before the specified date and time."}
                      {notif.type === "Resume" &&
                        "Your resume requires verification. Upload an updated version for approval."}
                    </p>
      ...
      <Card>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {[
            {
              type: "Drive",
              desc: "New recruitment drives and registration deadlines",
              icon: "🎯",
            },
          ...
          {[
            {
              ...
              desc: "Get notified when new drives are added",
              enabled: true,
            },
            {
            ...
            { name: "Deadline Alerts", desc: "Alerts for approaching deadlines", enabled: true },
            {
              ...
              desc: "Notifications for received offers",
              enabled: true,
            },
            {
              name: "Training Announcements",
              desc: "Updates on training programs",
              enabled: false,
            },
