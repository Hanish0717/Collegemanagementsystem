...
export function AdminGrievance() {
  const [activeTab, setActiveTab] = useState<"redressal" | "antiragging" | "whistleblower">("redressal");
  ...
  const [raggingComplaints, setRaggingComplaints] = useState([
    { id: "RAG-01", offender: "Reported: Under Investigation", date: "2026-07-12", severity: "High", actionTaken: "Committee Review", status: "Active" }
  ]);

  const [whistleblowerRecords, setWhistleblowerRecords] = useState([
    { id: "WB-901", department: "Finance Audit", reportText: "Discrepancy in lab chemical invoice allocations.", date: "2026-07-15", priority: "Critical" },
    { id: "WB-902", department: "Examination Cell", reportText: "Potential leaking of minor exam question templates before approval.", date: "2026-07-16", priority: "High" }
  ]);
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === "antiragging" && (
        <Card>
          <div className="flex justify-between items-center mb-3">
            <h3 className="font-semibold text-slate-800 text-sm">Anti-Ragging Incident Complaint Registry</h3>
              ...
              <tbody className="divide-y divide-slate-100">
                {raggingComplaints.map(complaint => (
                  <tr key={complaint.id}>
                    ...
                    <td className="py-3 font-mono text-slate-500 font-semibold">{complaint.date}</td>
                    <td className="py-3 text-center"><Badge tone="danger">{complaint.severity}</Badge></td>
                    <td className="py-3 font-semibold text-slate-600">{complaint.actionTaken}</td>
                    <td className="py-3 text-right">
                      {complaint.status === "Active" ? (
                        <button
                          onClick={() => {
                            setRaggingComplaints(prev => prev.map(c => c.id === complaint.id ? { ...c, status: "Closed", actionTaken: "Case Audited & Dismissed" } : c));
      ...
      {activeTab === "whistleblower" && (
        <Card>
          <div className="flex justify-between items-center mb-3">
            <h3 className="font-semibold text-slate-800 text-sm">Anonymous Whistleblower Vault (End-to-End Encrypted)</h3>
              ...
              <tbody className="divide-y divide-slate-100">
                {whistleblowerRecords.map(record => (
                  <tr key={record.id}>
                    ...
                    <td className="py-3 text-slate-500 font-semibold italic">"{record.reportText}"</td>
                    <td className="py-3 font-mono text-slate-500 font-semibold">{record.date}</td>
                    <td className="py-3 text-right"><Badge tone="danger">{record.priority}</Badge></td>
                  </tr>
                ))}
              </tbody>
            </table>
