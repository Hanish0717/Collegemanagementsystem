...
export async function recordDonation(payload: { alumniId: string; amount: number; cause: string; transactionId?: string }) {
...
export async function createSuccessStory(payload: { alumniId: string; title: string; content: string; category: string }) {
  const response = await api.post("/api/alumni/stories", payload);
  return response.data;
}

export async function sendAnnouncement(payload: { type: "Email" | "SMS" | "WhatsApp"; recipient: string; subject?: string; message: string; sentBy?: string }) {
...
export async function sendAlumniMessage(senderId: string, receiverId: string, content: string, fileUrl?: string) {
  const response = await api.post("/api/alumni/chat/send", { senderId, receiverId, content, fileUrl });
  return response.data;
}
