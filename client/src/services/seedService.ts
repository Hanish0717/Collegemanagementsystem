...
export async function seedHostelsIfEmpty(): Promise<void> {
  try {
    const { count } = await supabase.from('hostels').select('id', { count: 'exact', head: true });
      ...
      const blocks = [
        {
          hostel_id: hostel.id,
          name: 'Block A',
          total_rooms: Math.floor(hostelData.total_rooms / 2),
        },
        {
          hostel_id: hostel.id,
          name: 'Block B',
          total_rooms: hostelData.total_rooms - Math.floor(hostelData.total_rooms / 2),
        },
      ];
...
const DEFAULT_NOTIFICATIONS = [
  {
    id: `HN-${Date.now() % 1000000}-1`,
    title: 'Hostel fee payment reminder — Q1 2026',
    type: 'Fee',
    time: '1h ago',
    unread: true,
  },
  {
    id: `HN-${Date.now() % 1000000}-2`,
    title: 'New room allocation complaint submitted',
    type: 'Complaint',
    time: '2h ago',
    unread: true,
  },
  {
    id: `HN-${Date.now() % 1000000}-3`,
    title: 'Hostel policy update: Gate closing time changed to 10 PM',
    type: 'Policy',
    time: '1d ago',
    unread: true,
  },
  {
    id: `HN-${Date.now() % 1000000}-4`,
  ...
  {
    id: `HN-${Date.now() % 1000000}-5`,
    title: 'Maintenance work scheduled: Water supply cut on 3rd floor',
    type: 'Maintenance',
    time: '3d ago',
    unread: false,
  },
  {
    id: `HN-${Date.now() % 1000000}-6`,
    title: 'New resident check-in: Room A101',
    type: 'Info',
    time: '3d ago',
    unread: false,
  },
