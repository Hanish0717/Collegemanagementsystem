...
describe('fetchStats edge cases', () => {
  it('standard aggregates (rooms + allocations + fees)', async () => {
    ...
    mockResponses['hostel_blocks'] = () => ({ data: [{ total_rooms: 5, occupants: 3 }], error: null });
    mockResponses['hostel_allocations'] = (_: any, opts: any) => ({ count: 7, data: null, error: null });
    mockResponses['hostel_complaints'] = (_: any, opts: any) => ({ count: 2, data: null, error: null });
    mockResponses['hostel_fees'] = () => ({ data: [{ amount_paid: 1000, paid_amount: 1000, total_amount: 1000, pending_amount: 0, payment_status: 'Paid', status: 'Paid' }], error: null });
    mockResponses['hostel_visitors'] = (_: any, opts: any) => ({ count: 4, data: null, error: null });
  ...
  it('no rooms and no blocks returns zeros', async () => {
    ...
    mockResponses['hostel_allocations'] = (_: any, opts: any) => ({ count: 0, data: null, error: null });
    mockResponses['hostel_complaints'] = (_: any, opts: any) => ({ count: 0, data: null, error: null });
    mockResponses['hostel_fees'] = () => ({ data: [], error: null });
    mockResponses['hostel_visitors'] = (_: any, opts: any) => ({ count: 0, data: null, error: null });
  ...
  it('blocks-only data aggregates correctly', async () => {
    ...
    mockResponses['hostel_blocks'] = () => ({ data: [{ total_rooms: 12, occupants: 3 }, { total_rooms: 8, occupants: 2 }], error: null });
    mockResponses['hostel_allocations'] = (_: any, opts: any) => ({ count: 0, data: null, error: null });
    mockResponses['hostel_complaints'] = (_: any, opts: any) => ({ count: 0, data: null, error: null });
    mockResponses['hostel_fees'] = () => ({ data: [], error: null });
    mockResponses['hostel_visitors'] = (_: any, opts: any) => ({ count: 0, data: null, error: null });
  ...
  it('counts overdue fees correctly', async () => {
    ...
    mockResponses['hostel_fees'] = () => ({ data: [{ amount_paid: 0, paid_amount: 0, total_amount: 1500, pending_amount: 1500, payment_status: 'Pending', status: 'Pending', due_date: '2020-01-01' }], error: null });
    mockResponses['hostel_visitors'] = (_: any, opts: any) => ({ count: 0, data: null, error: null });
