...
export async function createDrive(
  payload: Omit<DriveItem, 'id' | 'studentCount' | 'rounds'>,
): Promise<DriveItem> {
...
export async function fetchTrainingPrograms(): Promise<any[]> {
  const { data } = await api.get<{ success: boolean; data: any[] }>('/api/placement/training');
  return data.data;
}
