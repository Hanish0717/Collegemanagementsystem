...
export async function addDepartment(
  dept: Omit<Department, 'id'> & { id: string },
): Promise<Department> {
...
export async function fetchCourses(): Promise<Course[]> {
  const { data } = await api.get<{ success: boolean; data: Course[] }>('/api/super-admin/courses');
  return data.data;
}
...
export async function fetchBackups(): Promise<{ backups: Backup[]; settings: boolean[] }> {
  const { data } = await api.get<{
    success: boolean;
    data: { backups: Backup[]; settings: boolean[] };
  }>('/api/super-admin/backups');
  return data.data;
}

export async function createBackup(): Promise<Backup> {
  const { data } = await api.post<{ success: boolean; data: Backup }>('/api/super-admin/backups');
  return data.data;
}
...
export async function fetchAutomations(): Promise<{
  ...
  const { data } = await api.get<{
    success: boolean;
    data: { automations: Automation[]; logs: AutomationLog[] };
...
export async function fetchNotifications(): Promise<{
  ...
  const { data } = await api.get<{
    success: boolean;
    data: { feed: NotificationItem[]; categories: boolean[] };
...
export async function fetchSecurityLogs(): Promise<{
  ...
  const { data } = await api.get<{
    success: boolean;
    data: { logs: SecurityLog[]; alerts: SecurityAlert[] };
...
export async function fetchSystemSettings(): Promise<{
  profile: any;
  securityOpts: boolean[];
  notifOpts: boolean[];
}> {
  const { data } = await api.get<{
    success: boolean;
    data: { profile: any; securityOpts: boolean[]; notifOpts: boolean[] };
...
export async function fetchSystemConfig(): Promise<{
  ...
  const { data } = await api.get<{
    success: boolean;
    data: { toggles: { [key: string]: boolean }; institution: any };
...
export async function fetchReportsData(filter?: string): Promise<any> {
  ...
  const { data } = await api.get<{ success: boolean; data: any }>('/api/super-admin/reports/data', {
    params,
  });
  return data.data;
}
...
export async function downloadReportCSV(reportName: string): Promise<Blob> {
  ...
  const { data } = await api.get<Blob>(`/api/super-admin/reports/download/${type}`, {
    responseType: 'blob',
  });
  return data;
}
...
export async function fetchUsers(): Promise<User[]> {
  const { data } = await api.get<{ success: boolean; data: User[] }>('/api/super-admin/users');
  return data.data;
}
