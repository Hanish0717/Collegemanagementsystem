...
export function DashboardLayout() {
  ...
  const [helpActiveTab, setHelpActiveTab] = useState<
    'getting-started' | 'role-guide' | 'faqs' | 'support'
  >('getting-started');
  ...
  const handleNotificationClick = () => {
    const notifRoute =
      role.nav.find((item) => item.label === 'Notifications')?.to ?? '/dashboard/notifications';
    navigate({ to: notifRoute });
  };

  const handleProfileClick = () => {
    const settingsRoute =
      role.nav.find((item) => item.label.toLowerCase() === 'settings')?.to ?? '/dashboard/settings';
      ...
      <nav className="flex-1 overflow-y-auto p-3 space-y-1">
        {navItems.map((item) => {
          ...
          if (item.children && (!collapsed || isMobile)) {
            ...
            const isOpen =
              openSubmenus[submenuKey] !== undefined ? openSubmenus[submenuKey] : anyChildActive;
            return (
              <div key={item.to}>
                      ...
                      return (
                        <Link
                          ...
                          search={
                            subHasQuery
                              ? Object.fromEntries(
                                  new URL(sub.to, window.location.origin).searchParams,
                                )
                              : undefined
                          }
                        ...
                        >
                          <span className="size-3.5">
                            {sub.icon ? <sub.icon className="size-3" /> : null}
                          </span>
                ...
                <div className="absolute left-0 right-0 top-full mt-2 bg-background border rounded-xl shadow-lg z-50 p-2 max-h-80 overflow-y-auto animate-in fade-in slide-in-from-top-2 duration-150">
                  ...
                  {suggestions.map((item, idx) => (
                    <div
                      ...
                      className={`p-2.5 rounded-lg text-xs cursor-pointer transition flex justify-between items-center ${
                        idx === activeIndex
              ...
              <div className="flex-1 p-6 overflow-y-auto space-y-4">
                {helpActiveTab === 'getting-started' && (
                  <div className="space-y-4 animate-in fade-in duration-150 text-left">
                    ...
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      Our platform is a state-of-the-art campus dashboard designed to integrate
                      academic portals, library inventories, hostel warden controls, financial
                      records, and live transit monitoring.
                    </p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-2">
                      <div className="p-3 border rounded-xl bg-accent/10 space-y-1">
                        ...
                        <span className="text-[11px] text-muted-foreground block">
                          Uses custom-harmonized dark mode support, HSL palettes, and responsive
                          side drawers.
                        </span>
                      </div>
                      <div className="p-3 border rounded-xl bg-accent/10 space-y-1">
                        <span className="font-bold text-xs text-foreground block">
                          🛡️ Role-Based Scope Security
                        </span>
                        <span className="text-[11px] text-muted-foreground block">
                          Access permissions are automatically sandboxed depending on your logged-in
                          credentials.
                        </span>
                      </div>
                    </div>
                    <div className="p-3.5 bg-yellow-500/10 border border-yellow-500/20 text-yellow-600 dark:text-yellow-400 rounded-xl text-[11px] leading-relaxed">
                      <strong>💡 Quick Tip:</strong> Click the role badge with your name at the
                      top-right corner to see a brief list of the features available to your account
                      level.
                    </div>
                  </div>
                )}
                    ...
                    {role.id === 'warden' && (
                      <div className="space-y-3.5">
                        <div className="border-l-2 border-teal-500 pl-3 space-y-1">
                          <span className="font-bold text-xs text-foreground block">
                            🔑 Room Allocations & Transfers
                          </span>
                          <span className="text-[11px] text-muted-foreground block">
                            Navigate to **Hostel** &rarr; **Rooms** or click **New** &rarr;
                            **Allocate Room** to assign beds. The system checks room capacity limits
                            and prevents double-booking a resident in active rooms.
                          </span>
                        </div>
                        <div className="border-l-2 border-teal-500 pl-3 space-y-1">
                          <span className="font-bold text-xs text-foreground block">
                            📋 Resident Directory Control
                          </span>
                          <span className="text-[11px] text-muted-foreground block">
                            Manage check-ins, check-outs, status updates (Active, Vacated), and
                            resident profiles. Only active resident profiles increment the occupied
                            counts for rooms.
                          </span>
                        </div>
                        <div className="border-l-2 border-teal-500 pl-3 space-y-1">
                          ...
                          <span className="text-[11px] text-muted-foreground block">
                            Add detailed visitor records including name, student contact, relation,
                            and time constraints. You can mark a visitor checked-out when they
                            leave.
                          </span>
                        </div>
                        <div className="border-l-2 border-teal-500 pl-3 space-y-1">
                          ...
                          <span className="text-[11px] text-muted-foreground block">
                            Keep track of unpaid dues, register paid receipts, and monitor financial
                            transactions. Always generate receipts with valid transaction IDs.
                          </span>
                        </div>
                      </div>
                    )}
                    ...
                    {role.id === 'student' && (
                      <div className="space-y-3.5">
                        <div className="border-l-2 border-indigo-500 pl-3 space-y-1">
                          ...
                          <span className="text-[11px] text-muted-foreground block">
                            Check class timetables, print grade cards, and monitor your cumulative
                            GPA trends.
                          </span>
                        </div>
                        <div className="border-l-2 border-indigo-500 pl-3 space-y-1">
                    ...
                    {role.id === 'faculty' && (
                      <div className="space-y-3.5">
                        <div className="border-l-2 border-violet-500 pl-3 space-y-1">
                          ...
                          <span className="text-[11px] text-muted-foreground block">
                            Click on the calendar classes to mark daily student presence. Ensure to
                            submit records before midnight.
                          </span>
                        </div>
                        <div className="border-l-2 border-violet-500 pl-3 space-y-1">
                          <span className="font-bold text-xs text-foreground block">
                          ...
                          <span className="text-[11px] text-muted-foreground block">
                            Upload classroom lecture notes, assignment tasks, or event notices to
                            your specific student courses.
                          </span>
                        </div>
                      </div>
                    )}
                    ...
                    {role.id !== 'warden' && role.id !== 'student' && role.id !== 'faculty' && (
                      <div className="space-y-3.5">
                        <div className="border-l-2 border-primary pl-3 space-y-1">
                          <span className="font-bold text-xs text-foreground block">
                            📈 Standard Operations Scope
                          </span>
                          <span className="text-[11px] text-muted-foreground block">
                            Use the sidebar navigation tabs to view registers, generate data tables,
                            or download audit reports.
                          </span>
                        </div>
                        <div className="border-l-2 border-primary pl-3 space-y-1">
                          <span className="font-bold text-xs text-foreground block">
                            ✉️ Alerts Broadcaster
                          </span>
                          <span className="text-[11px] text-muted-foreground block">
                            Check and send high priority notices to user groups under your
                            designated system panel.
                          </span>
                        </div>
                      </div>
                    )}
                  ...
                  <div className="space-y-4 animate-in fade-in duration-150 text-left">
                    ...
                    <div className="space-y-3">
                      <div className="p-3 border rounded-xl space-y-1 hover:bg-muted/10 transition">
                        <span className="font-bold text-xs text-foreground block">
                          Q: How do I switch my active role view?
                        </span>
                        <span className="text-[11px] text-muted-foreground block leading-relaxed">
                          A: Since this is a demo sandbox environment, you can switch active roles
                          using the role selectors or your profile profile. Click the user badge to
                          view configurations.
                        </span>
                      </div>
                      <div className="p-3 border rounded-xl space-y-1 hover:bg-muted/10 transition">
                        <span className="font-bold text-xs text-foreground block">
                          Q: What happens when a room reaches maximum capacity?
                        </span>
                        <span className="text-[11px] text-muted-foreground block leading-relaxed">
                          A: The database validations will prevent further student assignments to
                          that room and raise a warning notification message. You must transfer
                          occupants or assign a vacant room.
                        </span>
                      </div>
                      <div className="p-3 border rounded-xl space-y-1 hover:bg-muted/10 transition">
                        <span className="font-bold text-xs text-foreground block">
                          Q: Where can I review system logs and automation triggers?
                        </span>
                        <span className="text-[11px] text-muted-foreground block leading-relaxed">
                          A: High-level automated system functions are accessible only to the Super
                          Admin role scope under the AI Automation Control and Security panels.
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                {helpActiveTab === 'support' && (
                  <div className="space-y-4 animate-in fade-in duration-150 text-left">
                    ...
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      If you encounter database timeouts, permission issues, or require custom
                      features, please contact our system administration team.
                    </p>
                    <div className="space-y-2.5 pt-2">
                      ...
                      <div className="flex items-center gap-3 p-3 border rounded-xl hover:bg-muted/10 transition">
                        ...
                        <div>
                          ...
                          <span className="text-[11px] text-muted-foreground">
                            Main Campus Building, Room 410A
                          </span>
                        </div>
                      </div>
                    </div>
